...
import type { MultisigInfo } from "./multisigInfo";
...
export class Account {
  ...
  fixAddress(networkType: NetworkType): void {
    this.address = PublicAccount.createFromPublicKey(
      this.publicKey,
      networkType
    ).address.plain();
  }
  ...
  removeNamespace(id: string): void {
    const index = this.namespaces.findIndex(
      (namespace) => namespace.idHex === id
    );
  ...
  findNamespaceNameByAsset(assetId: string): Namespace[] | null {
    const foundNamespaces = this.namespaces.filter(
      (ns) => ns.linkedId === assetId
    );
  ...
  getDirectChildMultisig(): string[] {
    const temp: MultisigInfo[] = this.multisigInfo.filter(
      (multiInfo) => multiInfo.level === -1
    );
  ...
  addNonHoldingAsset(asset: Asset) {
    const nonHoldingAssetsId = this.nonHoldingAssets.map(
      (asset) => asset.idHex
    );
...
export const setDefaultAccInStorage = (address: string) => {
