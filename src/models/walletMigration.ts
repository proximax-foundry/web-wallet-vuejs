...
export class WalletMigration {
  ...
  async runPatching() {
    while (this.version < latestPatchVersion) {
      const newPatchVersion = this.version + 1;
  ...
  static async patchV1() {
    while (
      !AppState.readyStates.get("chainProfile") &&
      !AppState.readyStates.get("checkSession")
    ) {
    ...
    if (storedWallets) {
      ...
      const storedWalletsObj: any = JSON.parse(storedWallets);
      const networkName = WalletMigration.getFistChainProfileWithNetworkType(
        networkState.availableNetworks,
        NetworkType.MAIN_NET
      );

      if (networkName) {
        WalletUtils.initFixOldFormat(
          storedWalletsObj,
          networkName,
          NetworkType.MAIN_NET
        );
  ...
  static doLocalStorageBackup(storageName: string, version: number) {
    const backupData = localStorage.getItem(storageName);
    if (backupData) {
      localStorage.setItem(`pre${version}-${storageName}`, backupData);
    }
  }

  static getFistChainProfileWithNetworkType(
    allNetworkName: string[],
