...
export class SwapUtils {
  ...
  static generateIncomingPdfCert = (
    ...
    const dateRow = 255,
      ...
      swapIDRow = 385,
      hashRow = 455,
      qrRpw = 515; //, signatureRow = 425, hashValueRow = 485
  ...
  static generateoutgoingPdfCert = (
    networkName: string,
    swapTimestamp: string,
    siriusAddress: string,
    swapId: string,
    ...
    const dateRow = 255,
      ...
      swapIDRow = 435,
      hashRow = 505,
      qrRpw = 565; //, signatureRow = 425, hashValueRow = 485
  ...
  static signTransaction(
    ...
    aggreateCompleteTransaction: AggregateTransaction
  ): SignedTransaction {
  ...
  static getOutgoing_SwapTransfer_URL = (
    baseUrl: string,
    tokenName?: string
  ): string => {
    if (tokenName) {
  ...
  static checkTokenBalance = (url: string, tokenName: string): Promise<any> => {
    return fetch(`${url}/checkTokenBalance/${tokenName}`)
      .then((res) => res.json())
      .then((data) => {
        return data;
      });
  ...
  static getOutgoing_SwapCheckByTxID_URL = (
    baseUrl: string,
    txID: string
  ): string => {
  ...
  static getOutgoing_SwapCheckByID_URL = (
    baseUrl: string,
    swapId: string
  ): string => {
  ...
  static getIncoming_ETHSwapTransfer_URL = (
    baseUrl: string,
    tokenName: string
  ): string => {
  ...
  static getIncoming_BSCSwapTransfer_URL = (
    baseUrl: string,
    tokenName: string
  ): string => {
    return `${baseUrl}/${tokenName}/transfer`;
  ...
  static getIncoming_OldETHSwapTransfer_URL = (
    baseUrl: string,
    tokenName: string
  ): string => {
  ...
  static getIncoming_OldBSCSwapTransfer_URL = (
    baseUrl: string,
    tokenName: string
  ): string => {
  ...
  static fetchOldETHServiceInfo = async (
    baseUrl: string,
    tokenName: string
  ): Promise<paramResponse> => {
    const response = await fetch(`${baseUrl}/expx/${tokenName}/service-info`);
    let data = "";
    if (response.status == 200) {
      data = await response.json();
    }
    const returnResponse: paramResponse = {
  ...
  static fetchOldBSCServiceInfo = async (
    baseUrl: string,
    tokenName: string
  ): Promise<paramResponse> => {
  ...
  static fetchAllSwapServiceInfo = async (
    baseUrl: string
  ): Promise<paramResponse> => {
  ...
  static fetchTokenServiceInfo = async (
    baseUrl: string,
    tokenName: string
  ): Promise<paramResponse> => {
