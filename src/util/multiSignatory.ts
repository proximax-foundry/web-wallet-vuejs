...
const getAggregateFee = async (publicKey :string,addedCosigners: string[],numApprove :number,numDelete :number,removeCosign? :string[]) :Promise<string>=>{
  ...
  let cosignatory 
    ...
    let publicAcc = PublicAccount.createFromPublicKey(publicKey,networkType)
...
async function modifyMultisigAccount(coSign :string[], removeCosign :string[], numApproveTransaction :number, numDeleteUser :number, cosigners :{address :string}[] , multisigAccount :WalletAccount | OtherAccount, walletPassword :string) :Promise<boolean> {
  ...
  let publicAcc = PublicAccount.createFromPublicKey(multisigAccount.publicKey,networkType)
