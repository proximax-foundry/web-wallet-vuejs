...
const getLinkNamespaceToAddressTransactionFee = (isMultisig :boolean,namespaceAddress: string, namespaceId: string, linkType: string) :number => {
  ...
  if(!isMultisig){
    ...
    const aggregateBondedTx = AppState.buildTxn.aggregateBonded([linkAddressToNamespaceTx.toAggregate(PublicAccount.createFromPublicKey(publicKey,AppState.networkType, 1))])
...
const getDelegateFee = (isMultisig: boolean) :number=>{
  ...
  if(!isMultisig){
    ...
    let innerTx = [delegateTx.toAggregate(PublicAccount.createFromPublicKey(publicKey,AppState.networkType, 1))];
