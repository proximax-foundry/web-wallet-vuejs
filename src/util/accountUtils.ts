...
export class AccountUtils {
  ...
  static getNamespaceListByAddress = (address: string): Namespace[] => {
    ...
    const findAcc = walletState.currentLoggedInWallet.accounts.find(account => account.address === address)
    if (findAcc) {
      namespacelist = findAcc.namespaces.filter(namespace => namespace.active === true);
      return namespacelist
    }
    const findOtherAcc = walletState.currentLoggedInWallet.others.find(account => account.address == address)
    if (findOtherAcc) {
      namespacelist = findOtherAcc.namespaces.filter(namespace => namespace.active === true);
      return namespacelist
    }
  ...
  static getContact = () => {
    ...
    const wallet = walletState.currentLoggedInWallet;
    let contact: { value: string, label: string }[] = [];
  ...
  static namespaceOption = (address: string, linkOption: string) => {
    ...
    let namespacelist = this.getNamespaceListByAddress(address);
  ...
  static getAccountDetail = (senderAddress: string, walletPassword: string): Account => {
    const wallet = walletState.currentLoggedInWallet
    if (!wallet) {
      throw new Error("Service unavailable")
    }
    const accountAddress = Address.createFromRawAddress(senderAddress);
    const accountDetails = wallet.accounts.find((account) => account.address == accountAddress.plain());
  ...
  static linkNamespaceToAddressTransaction = (namespaceID: string, linkType: string, namespaceAddress: string): AddressAliasTransaction => {
  ...
  static delegateTransaction = (publicKey: string, delegateAction: LinkAction): AccountLinkTransaction => {
  ...
  static getLinkNamespaceToAddressTransactionFee = (isMultisig: boolean, namespaceAddress: string, namespaceId: string, linkType: string): number => {
    const linkAddressToNamespaceTx = this.linkNamespaceToAddressTransaction(namespaceId, linkType, namespaceAddress);
  ...
  static linkNamespaceToAddress = (selectedCosign: string, isMultisig: boolean, multisigAccount: WalletAccount | OtherAccount, walletPassword: string, namespaceID: string, linkType: string, namespaceAddress: string) :boolean => {

    if (!walletState.currentLoggedInWallet || !networkState.currentNetworkProfile) {
      throw new Error("Service unavailable")
    }
    const namespaceTransaction = this.linkNamespaceToAddressTransaction(namespaceID, linkType, namespaceAddress);
  ...
  static createDelegateTransaction = (selectedCosign: string, isMultisig: boolean, multisigAccount: WalletAccount, walletPassword: string, accPublicKey: string, delegateAction: LinkAction): boolean => {
    if (!walletState.currentLoggedInWallet || !networkState.currentNetworkProfile) {
      throw new Error("Service unavailable")
    }
    let delegateTx = this.delegateTransaction(accPublicKey, delegateAction)
