...
export class AddressBookUtils {
  static verifyNetworkAddressEqualsNetwork(
    currentPrimaryAdd: string,
    add: string
  ): boolean {
    if (
      (currentPrimaryAdd.length === 40 || currentPrimaryAdd.length === 46) &&
      (add.length === 40 || add.length === 46)
    ) {
  ...
  static verifyNetworkAddress(currentAdd: string, add: string): NetworkAddress {
    ...
    const verification: NetworkAddress = {
      isPassed: false,
      errMessage: "",
    };
    ...
    if (address !== null && address !== undefined && address.length === 40) {
      if (
        this.verifyNetworkAddressEqualsNetwork(currentAdd, add.toUpperCase())
      ) {
