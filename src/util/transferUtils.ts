...
export class TransferUtils {
  static test_publicKey = "0".repeat(64);
  static test_address = PublicAccount.createFromPublicKey(
    this.test_publicKey,
    AppState.networkType
  ).address.plain();

  static getAccInfo = async(address: string): Promise<PublicAccount> =>{
  ...
  static createTransaction = async (
    ...
    let msg;
    if (encryptedMsg) {
      const accountInfo = await TransferUtils.getAccInfo(recipientAddress);
  ...
  static calculate_aggregate_fee = (
    message: string,
    amount: string,
    mosaic: { id: string; amount: string }[]
  ): number => {
    ...
    const mosaics = TransferUtils.getMosaic(amount, mosaic);
    const transferTransaction = transactionBuilder
      .transferBuilder()
      .recipient(Address.createFromRawAddress(TransferUtils.test_address))
      .mosaics(mosaics)
      .message(PlainMessage.create(message))
      .build();
    const innerTxn = [
      transferTransaction.toAggregate(
        PublicAccount.createFromPublicKey(TransferUtils.test_publicKey, AppState.networkType)
      ),
    ];
