<template>
  <div>
    <form @submit.prevent="login">
      <fieldset>
        <div
          class="w-8/12 text-center mr-auto ml-auto error error_box"
          v-if="err != ''"
        >
          {{ err }}
        </div>
        ...
        <div class="mt-3"></div>
        <div class="w-8/12 mx-auto">
          <SelectWallet v-model="selectedWallet" />
        </div>
        <div class="mt-3"></div>
        <div class="text-center">
          <PasswordInput
            class="w-8/12 block ml-auto mr-auto"
        ...
        <div class="text-center mt-2">
          <button
            type="submit"
            class="blue-btn bg-gray-primary py-2.5 w-8/12 disabled:opacity-50"
            :disabled="disableSignin || passwordCorrectPattern"
          >
            {{ $t("home.signIn") }}
          </button>
        </div>
...
import SelectWallet from "@/modules/home/components/SelectWallet.vue";
...
const selectedWallet = ref<{ value: string; label: string } | null>(null);
...
const login = async () => {
  ...
  if (needAPIKey.value && apikeyInput.value === "") {
    ...
    if (lockingTime) {
      if (currentTime < parseInt(lockingTime) + 5 * 60 * 1000) {
        showAPIKeyError.value = true;
        apikeyError.value =
          "too many failed attempt. Please try again in few minutes";
        return;
      } else {
    ...
    if (apiKeyOK) {
      ...
      let tryCount = parseInt(
        localStorage.getItem("login_locking_retry") ?? "0"
      );
      tryCount = tryCount + 1;
  ...
  if (!result) {
    ...
    let wallet = walletState.wallets.filterByNetworkNameAndName(
      networkState.chainNetworkName,
      selectedWallet.value.label
    );
    ...
    NetworkStateUtils.updateLastAccessNetworkName(
      networkState.chainNetworkName
    );
