    ...
    <form @submit.prevent="login">
      <fieldset>
        ...
        <div class='text-center'>
          <PasswordInput class='w-8/12 block ml-auto mr-auto' :placeholder="$t('general.password')"
            :errorMessage="$t('general.passwordRequired')" :showError="showPasswdError" v-model="walletPassword"
            icon="lock" />
        </div>
        <div class='text-center mt-3' v-if="needAPIKey">
          <HiddenInput class='w-8/12 block ml-auto mr-auto' placeholder="API key" :errorMessage="apikeyError"
            :showError="showAPIKeyError" v-model="apikeyInput" icon="lock" />
        </div>
        <div class="text-center mt-2">
          <button type="submit" class=" blue-btn bg-gray-primary py-2.5 w-8/12 disabled:opacity-50"
            :disabled="disableSignin || passwordCorrectPattern">{{ $t('home.signIn') }}</button>
        </div>
      </fieldset>
    </form>
  </div>
...
emitter.on('select-wallet', (wallet: string) => {
...
const login = async () => {
  ...
  if (needAPIKey.value && apikeyInput.value === "") {
    ...
    if (apiKeyOK) {
      ...
      const lockingRetry = localStorage.getItem("login_locking_retry")
      let tryCount = lockingRetry?parseInt(lockingRetry):  0 
      tryCount = tryCount + 1;
