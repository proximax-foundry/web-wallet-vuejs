<template>
  <div>
    <a @click="toggleModal = !toggleModal"><img v-if='!loginStatus' src="@/assets/img/globe-white.svg"
        class="h-4 w-4 inline-block relative mr-2 " style="top: -1px">{{ $i18n.locale.toUpperCase() }}</a>
        ...
        <div class="modal-popup-box m-2">
          ...
          <div class="lang-option">
            <div v-for="(lang, item) in options" :key="item" class="flex justify-between border-gray-300 border-b py-3">
              <div class="text-gray-800 text-xs">{{ lang.label }}</div><span v-if="lang.value == selected"
                class="text-gray-400 text-txs items-center font-semibold flex uppercase">{{ $t('general.current') }}</span><span
                class="cursor-pointer text-txs items-center flex font-semibold text-blue-500 uppercase"
                @click="changeLang(lang.value)" v-else>{{ $t('general.select') }}</span>
            </div>
          </div>
        </div>
      </div>
