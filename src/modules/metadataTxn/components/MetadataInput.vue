<template>
  <div :class="disabled?'opacity-50':''">
    <div class="border border-gray-200 px-2 py-1 h-14 rounded-md">
      ...
      <input v-if="!hex" class="number_input" :disabled="disabled" @input="$emit('update:modelValue', $event.target.value)" :value="modelValue" type="text"  @focus="$event.target.select()" v-on:keypress="intCharOnly(event)" >
