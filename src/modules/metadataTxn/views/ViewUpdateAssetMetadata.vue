  ...
  <TransactionLayout :type="'metadata'">
    ...
    <template #navy>
      ...
      <div class="text-center">
        <router-link
          :to="{ name: 'ViewAccountAssets', params: { address: address } }"
...
export default {
  ...
  setup(props) {
    ...
    const handleParamTargetId = async () => {
      if (
        props.targetId &&
        props.targetId.length === 16 &&
        Convert.isHexString(props.targetId)
      ) {
        ...
        selectedAcc.value =
          walletState.currentLoggedInWallet.accounts.find(
            (acc) => acc.publicKey === assetInfo.owner.publicKey
          ) ||
    ...
    const updateMetadata = async () => {
      ...
      let aggregateTx = AppState.buildTxn
        ...
        .innerTransactions([
          mosaicMetadataTransaction.toAggregateV1(targetPublicAccount.value),
        ])
        .build()
        .serialize();
      ...
      if (targetAccIsMultisig.value) {
        ...
        unsignedTxnPayload = mosaicMetadataTransaction
