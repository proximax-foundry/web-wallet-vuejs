      ...
      <div class="border-2 border-t-0 filter shadow-lg lg:grid lg:grid-cols-5">
        <div class="lg:col-span-3 py-6 px-6">
          ...
          <div v-if="scopedMetadataKeySelectable">
            <MetadataInput :hex="scopedMetadataKeyType == 2" class="mt-5" v-model="inputScopedMetadataKey"
              ...
              :toolTip="`${scopedMetadataKeyType == 1 ? 'Accepts up to 8 bytes' : 'Accepts up to 16 hexadecimals'}`"
        ...
        <div class='bg-navy-primary p-6 lg:col-span-2'>
          ...
          <div class="text-center">
            <router-link :to="{ name: 'ViewAccountMetadata', params: { address: accountAddress } }"
...
const fetchExistingKey = () => {
  ...
  metadataQueryParams.targetKey = targetPublicAccount.value as PublicAccount
...
const handleParamTargetPublicKey = () => {
  ...
  if (props.targetPublicKey && props.targetPublicKey.length === 64 && Convert.isHexString(props.targetPublicKey)) {
...
const updateMetadata = async () => {
  ...
  let aggregateTx = targetAccIsMultisig.value ?
    aggregateTxnBuilder.innerTransactions([accountMetadataTransaction.toAggregate(targetPublicAccount.value as PublicAccount)]).build() :
    AppState.buildTxn.aggregateCompleteBuilder().innerTransactions([accountMetadataTransaction.toAggregate(targetPublicAccount.value as PublicAccount)]).build()
  ...
  if (MultisigUtils.getCosignerInWallet(props.targetPublicKey).cosignerList.length) {
    if (accounts.value) {
      return MultisigUtils.getCosignerInWallet(props.targetPublicKey).cosignerList.map(cosigner => {
        let foundCosigner = accounts.value.find(acc => acc.publicKey == cosigner) as { name: string; balance: number; address: string; publicKey: string; isMultisig: boolean; }
        return {
