...
export default {
  ...
  setup(props) {
    ...
    const accountAddress = computed(() =>
      selectedAcc.value ? selectedAcc.value.address : "0".repeat(40)
    );
    const handleParamTargetPublicKey = () => {
      if (
        props.targetPublicKey.length === 64 &&
        Convert.isHexString(props.targetPublicKey)
      ) {
        targetPublicAccount.value = PublicAccount.createFromPublicKey(
          props.targetPublicKey,
      ...
      selectedAcc.value =
        walletState.currentLoggedInWallet.accounts.find(
          (acc) => acc.publicKey === props.targetPublicKey
        ) ||
    ...
    const updateMetadata = async () => {
      ...
      let accountMetadataTransaction = txnBuilder
        ...
        .build();
      let aggregateTxPayload = AppState.buildTxn
        .aggregateCompleteBuilder()
        .innerTransactions([
          accountMetadataTransaction.toAggregateV1(targetPublicAccount.value),
        ])
        .build()
        .serialize();
      ...
      if (targetAccIsMultisig.value) {
        ...
        unsignedTxnPayload = accountMetadataTransaction
    ...
    const cosigners = computed(() => {
      ...
      if (
        MultisigUtils.getCosignerInWallet(props.targetPublicKey).cosignerList
          .length
      ) {
        if (accounts.value) {
