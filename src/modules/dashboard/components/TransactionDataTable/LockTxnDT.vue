      ...
      <Column style="width: 200px" v-if="!wideScreen">
        <template #body="{data}">
          ...
          <div>
            ...
            <div class="uppercase font-bold text-txs" v-tooltip.bottom="{ value:'<tiptext>' + $t('general.approximately') + ' ' + durationTime(data.duration) + ' ' + $t('general.day',durationTime(data.duration)) + '</tiptext>', escape: true}">{{data.duration ? data.duration + ' '+ $t('general.block',data.duration) : "-"}}</div>
          </div>
          <div>
