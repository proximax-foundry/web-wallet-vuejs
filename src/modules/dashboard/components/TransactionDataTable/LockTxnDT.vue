  ...
  <div>
    <DataTable :value="transactions" :paginator="true" :rows="10" scrollDirection="horizontal"
        ...
        <template #body="{ data }">
          ...
          <div>
            ...
            <div class="uppercase font-bold text-txs"
              v-tooltip.bottom="{ value: '<tiptext>' + $t('general.approximately') + ' ' + durationTime(data.duration) + ' ' + $t('general.day', durationTime(data.duration)) + '</tiptext>', escape: true }">
              {{ data.duration ? data.duration + ' ' + $t('general.block', data.duration) : "-" }}</div>
          </div>
          <div>
      ...
      <Column :header="$t('general.duration')" headerStyle="width:40px;" v-if="wideScreen">
        <template #body="{ data }">
          <span class="text-txs"
            v-tooltip.bottom="{ value: '<tiptext>' + $t('general.approximately') + ' ' + durationTime(data.duration) + ' ' + $t('general.day', durationTime(data.duration)) + '</tiptext>', escape: true }">{{ data.duration
              ? data.duration + ' ' + $t('general.block', data.duration) : "-" }}</span>
        </template>
      </Column>
      <Column :header="$t('dashboard.refunded')" v-if="selectedGroupType === transactionGroupType.CONFIRMED && wideScreen"
