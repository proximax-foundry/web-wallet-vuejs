    ...
    <DataTable
      ...
      <Column style="width: 200px" headerClass="invisible" v-if="!wideScreen">
        <template #body="{data}">
          ...
          <div>
            <div class="uppercase text-xxs text-gray-300 font-bold mb-1 mt-5">{{$t('dashboard.registerType')}}</div>
            <div class="flex items-center">
              <div class="uppercase font-bold text-txs mr-2">{{data.registerTypeName}}</div>
            </div>
          </div>
        </template>
      </Column>
      <Column style="width: 200px" headerClass="invisible" v-if="!wideScreen">
        <template #body="{data}">
          ...
          <div>
            ...
            <div class="flex items-center">
              <span class="text-txs font-bold">{{data.namespaceName}}</span>
            </div>
          </div>
          <div>
            <div class="uppercase text-xxs text-gray-300 font-bold mb-1 mt-5">{{$t('general.duration')}}</div>
            <div class="flex items-center">
              <span class="text-txs font-bold " v-tooltip.left="{ value:'<tiptext>' + $t('general.approximately') + ' ' + durationTime(data.duration) + ' ' + $t('general.day',durationTime(data.duration)) + '</tiptext>', escape: false}">{{data.duration ? data.duration + ' '+ $t('general.block',data.duration) : "-"}}</span>
            </div>
          </div>
        </template>
      </Column>
      ...
      <Column :header="$t('dashboard.registerType')" headerStyle="width:110px;" v-if="wideScreen">
        <template #body="{data}">
          <div class="text-txs">{{ data.registerTypeName }}</div>
        </template>
      </Column>
      <Column :header="$t('general.namespace')" headerStyle="width:40px;" v-if="wideScreen">
        <template #body="{data}">
          <span class="text-txs" >{{data.namespaceName}}</span>
        </template>
      </Column>
      <Column :header="$t('dashboard.parentNamespace')" headerStyle="width:40px;" v-if="wideScreen">
        <template #body="{data}">
          <span class="text-txs" >{{data.parentName ? data.parentName : ""}}</span>
        </template>
      </Column>
