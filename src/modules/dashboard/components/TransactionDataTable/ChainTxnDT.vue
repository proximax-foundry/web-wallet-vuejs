  ...
  <div>
    <DataTable
      ...
      <Column header="INFO" headerStyle="width:40px">
        <template #body="{data}">
          <span class="inline-block bg-blue-500 text-white py-1 px-1 my-1 mx-1" v-if="data.applyHeightDelta">{{ `Apply Height Delta: ${data.applyHeightDelta}` }}</span>
          <span class="inline-block bg-blue-500 text-white py-1 px-1 my-1 mx-1" v-if="data.newVersion">{{ `New version: ${data.newVersion}` }}</span>
          <span class="inline-block bg-blue-500 text-white py-1 px-1 my-1 mx-1" v-if="data.upgradePeriod">{{ `Upgrade period: ${data.upgradePeriod}` }}</span>
        </template>
      </Column>
