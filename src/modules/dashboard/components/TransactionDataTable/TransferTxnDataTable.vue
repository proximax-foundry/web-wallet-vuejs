<template>
  <div>
    <DataTable :value="transactions" :paginator="true" :rows="10" scrollDirection="horizontal"
      ...
      currentPageReportTemplate="" v-model:filters="filterAccount"
            ...
            <div class="uppercase font-bold text-txs">
              ...
              <span v-tooltip.right="Helper.createAddress(data.recipient).pretty()" v-else
                class="truncate inline-block text-txs">{{
                  walletState.currentLoggedInWallet?.convertAddressToNamePretty(data.recipient) }}</span>
            </div>
          </div>
        </template>
      </Column>
      ...
      <!-- <Column field="block" v-if="selectedGroupType === transactionGroupType.CONFIRMED && wideScreen" :header="$t('general.block')" headerStyle="width:110px;">
        <template #body="{data}">
      ...
      <Column :header="$t('general.amount')" headerStyle="width:110px;" v-if="wideScreen">
        <template #body="{ data }">
