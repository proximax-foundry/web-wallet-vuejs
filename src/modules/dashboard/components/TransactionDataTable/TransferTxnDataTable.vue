      ...
      <Column :header="$t('general.message')" headerStyle="width:40px;text-transform:uppercase" v-if="wideScreen">
        <template #body="{data}">
          <div>
            ...
            <img src="@/modules/dashboard/img/icon-encrypted-message.svg" v-tooltip.left="'<tiptitle>' + data.messageTypeTitle + '</tiptitle><tiptext>' + data.message + '</tiptext>'" class="inline-block" @click="showDecryptMessageModal = true" v-else-if="data.message !== 1 && data.messageType !== 0">
