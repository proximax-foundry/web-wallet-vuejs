<template>
  <div>
    <DataTable :value="transactions" :paginator="true" :rows="10" scrollDirection="horizontal"
          ...
          <div>
            ...
            <div v-if="data.isTakingOffer">
              <span v-bind:key="index" v-for="(exchangeOffer, index) in data.exchangeOffers">
                <span class="inline-block bg-green-200 font-bold text-green-700 text-txs rounded py-2 px-1 my-1"
      ...
      <Column :header="$t('dashboard.offer')" headerStyle="width:40px;" v-if="wideScreen">
        <template #body="{ data }">
          <div v-if="data.isTakingOffer">
            <span v-bind:key="index" v-for="(exchangeOffer, index) in data.exchangeOffers">
              <span class="inline-block bg-green-200 font-bold text-green-700 text-txs rounded py-2 px-1 my-1 mx-1"
                v-if="exchangeOffer.type === 'Sell'">
                {{ displayExchangeSDA(exchangeOffer) }}
              </span>
...
import type { TxnExchangeOffer } from "../../model/transactions/exchangeOffer";
...
const displayExchangeSDA = (exchangeOffer: TxnExchangeOffer) => {
  ...
  const { amount, assetNamespace, cost } = exchangeOffer
  let asset_div = (amount as number > 0) ? (amount + " ") : '';
  asset_div += (assetNamespace ? exchangeOffer.assetNamespace : exchangeOffer.assetId);
  asset_div += (cost as number > 0) ? (" - " + exchangeOffer.cost + " " + nativeTokenName.value) : '';
