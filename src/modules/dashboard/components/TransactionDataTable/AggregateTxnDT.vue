    ...
    <DataTable
      ...
      <Column style="width: 200px" headerClass="invisible" v-if="!wideScreen">
        <template #body="{data}">
          ...
          <div>
            <div class="uppercase text-xxs text-gray-300 font-bold mb-1 mt-5">{{$t('general.transaction',2)}}</div>
            <div class="flex items-center">
      ...
      <Column :header="$t('dashboard.txFee')" headerStyle="width:110px;" v-if="selectedGroupType === transactionGroupType.CONFIRMED && wideScreen" >
        <template #body="{data}">
          <div class="text-txs">{{ data.fee }} <b v-if="data.fee">{{ nativeTokenName }}</b></div>
        </template>
      </Column>
      <Column :header="$t('general.transaction',2)" headerStyle="width:40px;" v-if="wideScreen">
        <template #body="{data}">
