    ...
    <div class="lg:w-9/12 ml-2 mr-2 lg:ml-auto lg:mr-auto mt-5">
      ...
      <div class="border-2 border-t-0 filter shadow-lg lg:grid lg:grid-cols-3">
        ...
        <div class='bg-navy-primary p-6 lg:col-span-1'>
          ...
          <div class="mt-3"></div>
          <button v-if="selectAction == 'Link'" class="w-full blue-btn px-3 py-3 disabled:opacity-50 disabled:cursor-auto"
            @click="aliasAddressToNamespace" :disabled="disableCreate">{{ $t('general.linkToNamespace') }}</button>
          <button v-if="selectAction == 'Unlink'"
            class="w-full blue-btn px-3 py-3 disabled:opacity-50 disabled:cursor-auto" @click="aliasAddressToNamespace"
            :disabled="disableCreate">{{ $t('namespace.unlinkNamespace') }}</button>
...
const accountDisplayBalance = computed(() => {
  if (walletState.currentLoggedInWallet) {
    if (!isMultiSig.value) {
      ...
      if (findAccount) {
        return Helper.toCurrencyFormat(findAccount.balance, currentNativeTokenDivisibility.value);
      } else {
        return '0'
      }
    }
...
const aliasAddressToNamespace = () => {
  ...
  if (!WalletUtils.verifyWalletPassword(walletName, networkState.chainNetworkName, walletPassword.value)) {
    ...
    AccountUtils.linkNamespaceToAddress(selectedCosignPublicKey.value, isMultiSig.value, acc as WalletAccount | OtherAccount, walletPassword.value, selectNamespace.value, selectAction.value, namespaceAddress.value)
