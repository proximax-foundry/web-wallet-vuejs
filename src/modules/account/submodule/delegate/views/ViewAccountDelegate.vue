  ...
  setup(p) {
    ...
    const disableLinkBtn = computed(() => {
      if (
        onPartial.value ||
        fundStatus.value ||
        (!isCosigner.value && isMultisig.value)
      ) {
        return true;
      } else {
        return false;
    ...
    const verifyDelegateAcc = async () => {
      ...
      if (acc.value) {
        ...
        const accountInfo = await chainAPICall.accountAPI.getAccountInfo(
    ...
    const createDelegate = async () => {
      const account = WalletUtils.createAccountFromPrivateKey(
        privateKey.value,
        AppState.networkType
      );
      if (account) {
      ...
      if (delegateAcc.value !== "0".repeat(64)) {
        //unlink
        const delegateUnlinkTransaction = txBuilder
          .accountLinkBuilder()
        ...
        err.value = "";
      } else if (AccPublicKey.value != "") {
        //link
        const delegateLinkTransaction = txBuilder
          .accountLinkBuilder()
