...
export default {
  ...
  setup(p) {
    ...
    const cosignerAddress = (cosigner) =>
      Address.createFromPublicKey(cosigner, AppState.networkType)
        .plain()
        .substr(-4);
    ...
    const numApproveTransaction = ref(
      acc.value
        ? acc.value.multisigInfo.find((acc) => acc.level == 0).minApproval
        : 0
    );
    const numDeleteUser = ref(
      acc.value
        ? acc.value.multisigInfo.find((acc) => acc.level == 0).minRemoval
        : 0
    );
    const cosignerName = computed(
      () => {
        ...
        cosignaturies.value.forEach((publicKey) => {
          let address = Address.createFromPublicKey(
            publicKey,
            AppState.networkType
          ).plain();
          let contact = walletState.currentLoggedInWallet.contacts.find(
          ...
          if (contact) {
            ...
            name.push(
              wallet.accounts.find((acc) => acc.publicKey == publicKey).name
            );
          } else {
            let address = Address.createFromPublicKey(
              publicKey,
    ...
    const selectedCosignPublicKey = ref(
      walletCosignerList.value[0] ? walletCosignerList.value[0].publicKey : ""
    );
    ...
    const addCoSigButton = computed(() => {
      var status = false;
      if (
        accountBalance.value >= totalFee.value &&
        isCoSigner.value &&
        (!onPartial.value || enableEditPartial.value)
      ) {
    ...
    const clear = () => {
      ...
      numApproveTransaction.value = acc.value.multisigInfo.find(
        (acc) => acc.level === 0
      ).minApproval;
      numDeleteUser.value = acc.value.multisigInfo.find(
        (acc) => acc.level === 0
      ).minRemoval;
    ...
    const modifyAccount = async () => {
      ...
      cosigners.value.cosignerList.forEach((publicKey) => {
        signer.push({
          address: walletState.currentLoggedInWallet.accounts.find(
      ...
      let selectedCosignAddress =
        walletState.currentLoggedInWallet.accounts.find(
          (account) => account.publicKey == selectedCosignPublicKey.value
        ).address;
      passwordErr.value = "";
      ...
      TransactionState.unsignedTransactionPayload =
        modifyMultisigTransaction.serialize();
      ...
      if (walletState.currentLoggedInWallet) {
        if (findAcc(selectedCosignPublicKey.value)) {
          return Helper.toCurrencyFormat(
            findAcc(selectedCosignPublicKey.value).balance,
    ...
    const checkCosignBalance = computed(() => {
      let cosignBalance = findAcc(selectedCosignPublicKey.value)
        ? findAcc(selectedCosignPublicKey.value).balance
        : 0;
