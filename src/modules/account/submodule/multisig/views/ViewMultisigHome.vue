      ...
      <div class=' p-6 border-2 border-t-0 '>
        <div v-if="isMultisig" class="flex cursor-pointer">
          ...
          <router-link :to="{ name: 'ViewMultisigScheme', params: { address: address } }"
            class="border-2 border-blue-primary p-1 mb-3 w-16 text-blue-primary text-xs text-center font-semibold ">{{ $t('general.scheme') }}</router-link>
        </div>
          ...
          <div class="flex flex-col gap-2">
            <div v-for="(multisig, index) in multisigAccountsList" :key="index">
              <div class="border w-full cursor-pointer rounded-md p-3" @click="navigate(multisig.address)">
                ...
                <div class="flex">
                  <div :id="`multisigAddress${index}`" :copyValue="multisig.address" :title="multisig.address"
                    :copySubject="$t('general.address')" class="truncate md:text-clip md:w-auto text-txs font-bold mt-1">
                    {{ multisig.address }}</div>
                  <font-awesome-icon icon="copy" @mouseover="isHover = true" @mouseout="isHover = false"
                    :title="$t('general.copy')" @click="copy(`multisigAddress${index}`)"
                    class="ml-1 w-5 h-5 text-blue-link cursor-pointer "></font-awesome-icon>
                  <img v-if="findAccountWithAddress(multisig.address)" class="w-5 h-5 ml-auto"
                    src="@/assets/img/chevron_right.svg">
                </div>
              </div>
            </div>
          </div>
          <div v-if="!isCosigner" class='text-blue-primary text-xs text-center font-semibold'>{{ $t('general.ntgToShow') }}
          </div>
          <div v-if="!isCosigner"
...
const getAccountName = (publicKey: string) => {
  ...
  if (contact) {
    ...
    const findAcc =findAccount(publicKey) 
    return findAcc ? findAcc.name : `Cosigner-${convertAddress(publicKey).plain().substr(-4)}`
  }
}
let multisigAccountsList = computed(() => {
  ...
  let multisigAccountsList: { name: string, address: string }[] = []
...
let cosignerAccountsList = computed(() => {
  ...
  let cosignerAccountsList: { name: string, address: string }[] = []
