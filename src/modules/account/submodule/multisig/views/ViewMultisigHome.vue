      ...
      <div class=' p-6 border-2 border-t-0 '>
        <div v-if="isMultisig" class="flex cursor-pointer">
          <div
            ...
            {{ $t('general.multisig') }}</div>
          <router-link :to="{ name: 'ViewMultisigScheme', params: { address: address } }"
            class="border-2 border-blue-primary p-1 mb-3 w-16 text-blue-primary text-xs text-center font-semibold ">{{
              $t('general.scheme') }}</router-link>
        </div>
          ...
          <div class="flex flex-col gap-2">
            <div v-for="(cosigner, index) in cosignerAccountsList" :key="index">
              <div class="border w-full cursor-pointer rounded-md p-3" @click="navigate(cosigner.address)">
                ...
                <div class="flex">
                  <div :id="`cosignerAddress${index}`" :copyValue="cosigner.address" :copySubject="$t('general.address')"
                    :title="cosigner.address" class="truncate md:text-clip md:w-auto text-txs font-bold mt-1">
                    {{ cosigner.address }}</div>
                  <font-awesome-icon icon="copy" @mouseover="isHover = true" @mouseout="isHover = false"
                    :title="$t('general.copy')" @click="copy(`cosignerAddress${index}`)"
                    class="ml-2 w-5 h-5 text-blue-link cursor-pointer "></font-awesome-icon>
                  <img v-if="findAccountWithAddress(cosigner.address)" class="w-5 h-5 ml-auto"
                    src="@/assets/img/chevron_right.svg">
                </div>
              </div>
            </div>
          </div>
          <div v-if="!isMultisig" class='text-blue-primary text-xs text-center font-semibold'>{{ $t('general.ntgToShow')
          }}
          </div>
        ...
        <router-link
          :to="{ name: isMultisig ? 'ViewMultisigEditAccount' : 'ViewMultisigConvertAccount', params: { address: address } }"
          class="blue-btn py-2 px-2 ">{{ $t('multisig.manageCosignatories') }}</router-link>
        ...
        <div class='border p-4 mt-3'>
          <div v-if="multisigLength" class="w-full ">
            <button class="mr-5 w-32 blue-btn px-3 py-3 disabled:opacity-50 disabled:cursor-auto" @click="expandTree()">
              Expand All
            </button>
            <button class="w-32 blue-btn px-3 py-3 disabled:opacity-50 disabled:cursor-auto" @click="collapseTree()">
              Collapse All
            </button>
            <Tree v-model:expandedKeys="expandedKeys" :value="multisigAddress" :filter="true" filterMode="strict"
              @node-select="onNodeSelect" selectionMode="single" class="mt-4">
          ...
          <div v-if="!multisigLength" class='text-blue-primary text-xs text-center font-semibold'>
            {{ $t('general.ntgToShow') }}</div>
...
const navigate = (address) => {
  ...
  if (!findAccountWithAddress(address)) {
...
const multisigAddress = computed(() => {
  let multisigAccounts: { key: string, label: string, selectable: boolean, children: { key: string, label: string, selectable: boolean, data: string, type: string, balance: number | null }[] }[] = [];

  if (!AppState.isReady || !AppState.chainAPI || !acc.value) {
    return multisigAccounts
  }

  const levelBelowZero = [...acc.value.multisigInfo.filter(info => info.level < 0)]
  ...
  for (let i = 0; i < Math.abs(lowestLevel); i++) {
    const multisigAccCurrentLevel = levelBelowZero.filter((accounts) => accounts.level == -Math.abs(i + 1)).map(acc => WalletUtils.createAddressFromPublicKey(acc.publicKey, AppState.networkType).pretty())

    multisigAccounts.push({
      key: (i + 1).toString(),
      label: "Level " + (i + 1).toString(),
      selectable: false,
      children: multisigAccCurrentLevel.map((address, index) => {
        const findAccInWallet = walletState.currentLoggedInWallet.accounts.find(acc => acc.address == Address.createFromRawAddress(address).plain()) || walletState.currentLoggedInWallet.accounts.find(acc => acc.address == Address.createFromRawAddress(address).plain())
        return {
          type: "child",
          key: '0-' + index.toString(),
          balance: findAccInWallet ? findAccInWallet.balance : null,
          label: getAccountNameByAddress(Address.createFromRawAddress(address).plain()) ? getAccountNameByAddress(Address.createFromRawAddress(address).plain()) : address,
          data: address,
          selectable: true
        }
      })
...
const multisigLength = computed(() => multisigAddress.value.length)
...
const onNodeSelect = (node) => {
  ...
  if (!findAccountWithAddress(node.data)) {
