...
<div class="flex ml-auto flex-col w-40">
    ...
    <div v-if="toggleModal" class="relative ">
        <div class='absolute border border-t-0 z-20 bg-white w-full p-3 '>
            ...
            <div>
                <router-link class="hover:bg-gray-100" v-if="!otherAccount(address)" :to="{ name: 'ViewAccountMetadata', params: { address: address}}">Account Metadata</router-link>
                <router-link class="hover:bg-gray-100" v-else-if="otherAccount(address) && other_acc.type =='MULTISIG'" :to="{ name: 'ViewAccountMetadata', params: { address: address}}">Account Metadata</router-link>
                <div  v-else class="text-gray-300">Account Metadata</div>
            </div>
        </div>
    </div>
</div>
