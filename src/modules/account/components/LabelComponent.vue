<template>
    <div class="flex flex-col">
        <div @click="toggleSelection = !toggleSelection"
            class="border p-2 cursor-pointer w-52 bg-white text-xs justify-between flex">
            <div>Labels</div>
            <img v-if="!toggleSelection" src="@/modules/account/img/icon-arrow-down.svg" class="w-3 ml-2 h-3"
            ...
            <div class="absolute border-t-0 w-52 border bg-white ">
                <div class="flex flex-col max-h-44 overflow-auto">
                    <div v-for="(label, index) in labels" :key="index">
                        <div @click="!isHover ? filteredList[index] = !filteredList[index] : ''"
                            :class="`${filteredList[index] ? 'bg-blue-300 flex cursor-pointer p-2 hover:bg-blue-300 ' : 'flex cursor-pointer p-2 hover:bg-blue-300 '}`">
                            <font-awesome-icon icon="tag" class="w-4 h-4 mr-3 thin" />
                            <div class="text-xs mt-0.5">{{ label.name }}</div>
                            <font-awesome-icon icon="trash" @mouseover="isHover = true" @mouseout="isHover = false"
                                @click="removeLabel(index)" title="Remove label"
                                class='ml-auto w-3 h-3 cursor-pointer mt-0.5' />
                        </div>
                    </div>
                    <div @click="toggleModal = true; toggleSelection = false"
                        class="text-xs cursor-pointer flex gap-3 p-2 hover:bg-blue-300 ">
        ...
        <div class="modal-popup-box ">
            <div>
                ...
                <div class='text-center cursor-pointer text-xs font-semibold text-blue-link mt-2'
                    @click="toggleModal = false">{{ $t('general.cancel') }}</div>
            </div>
        </div>
    </div>
</template>
