<template>
  <div>
    <DataTable :value="transaction" :paginator="true" :rows="20" scrollDirection="horizontal" :alwaysShowPaginator="false"
        ...
        <template #body="{ data }">
          <div>
            ...
            <div class=" uppercase font-bold text-txs">
              <div class="flex items-center">
                <span class="text-txs text-blue-primary cursor-pointer" v-if="data.groupType != 'In Queue'"
                  @click="gotoHashExplorer(data.hash)" v-tooltip.right="data.hash">{{ data.hash.substring(0, 20)
                  }}...</span>
          ...
          <div>
            <div class="uppercase text-xxs text-gray-300 font-bold mb-1 mt-5"
              v-if="data.recipient != '' && data.recipient != null">Receipient</div>
            <div class="uppercase font-bold text-txs">
              ...
              <span v-tooltip.right="Helper.createAddress(data.recipient).pretty()" v-else
                class="truncate inline-block text-txs">{{ data.recipient.substring(0, 20) }}...</span>
            </div>
          </div>
        </template>
      </Column>
      ...
      <Column field="hash" :header="$t('dashboard.txHash')" headerStyle="width:100px" v-if="wideScreen">
        <template #body="{ data }">
          <div class="flex items-center">
            <span v-if="data.groupType != 'In Queue'" @click="gotoHashExplorer(data.hash)"
              class="cursor-pointer text-txs text-blue-primary" v-tooltip.bottom="data.hash">{{ data.hash.substring(0, 20)
              }}...</span>
        ...
        <template #body="{ data }">
          <div class="text-center">
            <img src="@/modules/dashboard/img/icon-proximax-logo-gray.svg" class="inline-block"
              ...
              v-tooltip.left="'<tiptitle>' + $t('general.sdaFull') + '</tiptitle><tiptext>' + displayAsset(data.sda) + '</tiptext>'">
            <span v-else>-</span>
          </div>
        </template>
      </Column>
        ...
        <template #body="{ data }">
          <div>
            <img src="@/modules/dashboard/img/icon-message.svg"
              v-tooltip.left="{ value: '<tiptitle>' + data.messageTypeTitle + '</tiptitle><tiptext>' + data.message + '</tiptext>', escape: true }"
      ...
      <Column headerStyle="width:50px" headerClass="invisible" v-if="!wideScreen">
        <template #body="{ data }">
          ...
          <router-link v-else-if="data.groupType == 'partial'"
            :to="{ name: 'ViewTransactionSign', params: { txnHash: data.hash } }"
            class="bg-orange-action text-white font-bold text-xxs text-center p-3 flex items-center justify-center">
...
import type { SDA } from '@/modules/dashboard/model/transactions/sda';
...
const displayAsset = async (assets :SDA[]) => {
  let asset_divs = '';
  if (assets.length > 0) {
...
const displayAssetDiv = async (asset :SDA ) => {
  if(!AppState.chainAPI){
    return ""
  }
  let otherAsset = await AppState.chainAPI.assetAPI.getMosaic(new MosaicId(asset.id) );
  let asset_div :string = "";
  let arrayAsset :MosaicId[] = []
  arrayAsset.push(new MosaicId(asset.id));
  ...
  if (nsAsset[0].names.length > 0) {
    ...
    asset_div = (asset.id + ' - ' + Helper.convertToExact(asset.amount, otherAsset.divisibility) + AppState.nativeToken.label);
  }
  return asset_div;
}
