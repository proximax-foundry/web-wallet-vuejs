<template>
    <img src="@/modules/account/img/edit-icon.svg" class="w-4 h-4 text-black cursor-pointer mt-1 ml-1"
    ...
    <div v-if="toggleModal" class="popup-outer-lang fixed flex z-50">
        <div class="modal-popup-box ">
            <div v-if="!isOther">
                ...
                <TextInput class="mt-3" :placeholder="$t('account.namePlaceholder')"
                    :errorMessage="$t('account.enterAccountName')" v-model="accountName" icon="wallet" />
                <div class="flex justify-center mt-3">
                    <button @click="changeName()"
                        class=' blue-btn font-semibold py-2 cursor-pointer text-center w-7/12 disabled:opacity-50 disabled:cursor-auto'
                        :disabled="disabledConfirm">{{ $t('general.confirm') }}</button>
                </div>
            </div>
            <div v-else-if="isOther && !isEdit">
                ...
                <div class="flex justify-center">
                    <button @click="saveContact()"
                        class=' blue-btn font-semibold py-2 cursor-pointer text-center w-7/12 disabled:opacity-50 disabled:cursor-auto'
                        :disabled="disabledAdd">{{ $t('general.add') }}</button>
                </div>
            </div>
            <div v-else>
                <div class="error error_box mb-3" v-if="err != ''">{{ err }}</div>
            ...
            <div class='text-center cursor-pointer text-xs font-semibold text-blue-link mt-2'
                @click="toggleModal = !toggleModal; err = ''">{{ $t('general.cancel') }}</div>
        </div>
    </div>
    <div @click="toggleModal = !toggleModal; err = ''" v-if="toggleModal"
...
const changeName = () => {
    ...
    if (accountName.value.trim()) {
        const exist_account = wallet.accounts.find((accName) => accName.name == accountName.value.trim());
        const exist_other_account = wallet.others.find((accName) => accName.name == accountName.value.trim());

        if (!exist_account && !exist_other_account) {
            const acc_index = wallet.accounts.findIndex((accAdd) => accAdd.address === props.address);
            if (acc_index == -1) {
                const other_acc_index = wallet.others.findIndex((accAdd) => accAdd.address === props.address);
                wallet.others[other_acc_index].name = accountName.value;
            }
            else {
                wallet.accounts[acc_index].name = accountName.value;
            }
...
const isEdit = computed(() => {
    if (!walletState.currentLoggedInWallet || !props.isOther) {
        return false
    }

    const findContact = walletState.currentLoggedInWallet.contacts.find(contact => contact.address == props.address)
    if (findContact) {
        return true
    }
    return false
...
const editContact = () => {
    ...
    const contact = wallet.contacts.find(contact => contact.address == props.address)
    if (!contact) {
        return
    }
    const contactIndex = wallet.contacts.findIndex((contact) => contact.address == props.address);
    ...
    if (findNameInTempContact != undefined || findNameInAcc != undefined) {
        ...
        wallet.updateAddressBook(contactIndex, new AddressBook(contactName.value.trim(), Address.createFromRawAddress(props.address).plain(), 'none'));
        walletState.wallets.saveMyWalletOnlytoLocalStorage(wallet);
        err.value = '';
        contactName.value = ''
        toggleModal.value = false
