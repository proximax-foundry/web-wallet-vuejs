      ...
      <div class="bg-white px-2 ">
        <div class="flex flex-col gap-3 mt-3">
          ...
          <div v-if="selectedTxnType === TransactionFilterType.ACCOUNT" class="flex items-center">
            <div class="h-3 w-3 bg-green-300 inline-block mr-1"></div> <span
              class="text-xs text-gray-500">{{ $t('dashboard.accountAdded') }}</span>
            <div class="h-3 w-3 bg-red-300 inline-block mr-1 ml-3"></div> <span
              class="text-xs text-gray-500">{{ $t('dashboard.accountRemoved') }}</span>
          </div>
          <div v-else-if="selectedTxnType === TransactionFilterType.EXCHANGE" class="flex items-center">
            <div class="h-3 w-3 bg-green-300 inline-block mr-1"></div> <span
              class="text-xs text-gray-500">{{ $t('dashboard.buyOffer') }}</span>
            <div class="h-3 w-3 bg-red-300 inline-block mr-1 ml-3"></div> <span
              class="text-xs text-gray-500">{{ $t('dashboard.sellOffer') }}</span>
          </div>
          <div v-if="selectedTxnType === TransactionFilterType.ASSET" class="flex items-center">
            <div class="h-3 w-3 bg-green-300 inline-block mr-1"></div> <span
              class="text-xs text-gray-500">{{ $t('general.enabled') }}</span>
            <div class="h-3 w-3 bg-red-300 inline-block mr-1 ml-3"></div> <span
              class="text-xs text-gray-500">{{ $t('general.disabled') }}</span>
          </div>


          <select v-model="selectedTxnType" @change="changeSearchTxnType"
            class="w-36 border border-gray-200 px-2 py-1 focus:outline-none">
            <option value="all" class="text-sm">All</option>
            <option v-bind:key="txnType.value" v-for="txnType in txnTypeList" :value="txnType.value" class="text-sm">{{
              txnType.label }}</option>
          </select>

        </div>
        <div v-if="boolIsTxnFetched" class="mt-3">
          <MixedTxnDataTable v-if="selectedTxnType === 'all'" :selectedGroupType="transactionGroupType.CONFIRMED"
            :transactions="mixedTransactions" :currentAddress="accAddress"></MixedTxnDataTable>
          <TransferTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.TRANSFER"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="transferTransactions"
            :currentAddress="accAddress"></TransferTxnDataTable>
          <AccountTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.ACCOUNT"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="accountTransactions"
            :currentAddress="accAddress"></AccountTxnDataTable>
          <AggregateTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.AGGREGATE"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="aggregateTransactions"
            :currentAddress="accAddress"></AggregateTxnDataTable>
          <AliasTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.ALIAS"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="aliasTransactions"
            :currentAddress="accAddress"></AliasTxnDataTable>
          <AssetTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.ASSET"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="assetTransactions"
            :currentAddress="accAddress"></AssetTxnDataTable>
          <ChainTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.CHAIN"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="chainTransactions"
            :currentAddress="accAddress"></ChainTxnDataTable>
          <ExchangeTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.EXCHANGE"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="exchangeTransactions"
            :currentAddress="accAddress"></ExchangeTxnDataTable>
          <LinkTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.LINK"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="linkTransactions"
            :currentAddress="accAddress"></LinkTxnDataTable>
          <LockTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.LOCK"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="lockTransactions"
            :currentAddress="accAddress"></LockTxnDataTable>
          <MetadataTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.METADATA"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="metadataTransactions"
            :currentAddress="accAddress"></MetadataTxnDataTable>
          <NamespaceTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.NAMESPACE"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="namespaceTransactions"
            :currentAddress="accAddress"></NamespaceTxnDataTable>
          <RestrictionTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.RESTRICTION"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="restrictionTransactions"
            :currentAddress="accAddress"></RestrictionTxnDataTable>
          <SecretTxnDataTable v-else-if="selectedTxnType === TransactionFilterType.SECRET"
            :selectedGroupType="transactionGroupType.CONFIRMED" :transactions="secretTransactions"
            :currentAddress="accAddress"></SecretTxnDataTable>
        </div>
        ...
        <a :href="linkToExplorer()" target=_new v-if="searchedTransactions.length == 10">
