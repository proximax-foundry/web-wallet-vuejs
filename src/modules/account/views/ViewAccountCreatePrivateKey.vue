  ...
  <div>
    <div class='flex cursor-pointer'>
      ...
      <router-link :to='{ name: "ViewAccountCreateSelectType" }'
        class='text-blue-primary text-xs mt-0.5'>{{ $t('general.back') }}</router-link>
    </div>
    <div class='border w-8/12 ml-auto mr-auto mt-6 filter shadow-lg'>
      ...
      <div class="w-8/12 ml-auto mr-auto mt-3">
        <PasswordInput :placeholder="$t('general.privateKey')" :errorMessage="$t('general.invalidPrivateKey')" icon="key"
          :showError="showPkError" v-model="privKey" />
        ...
        <PasswordInput class="mt-3" :placeholder="$t('general.enterPassword')"
          :errorMessage="$t('general.passwordRequired')" :showError="showPasswdError" icon="lock"
          v-model="walletPassword" />
      </div>
      <div class="flex justify-center mt-3">
        <button type="submit" class="blue-btn py-2 px-8 disabled:opacity-50" @click='create()'
          :disabled="disableCreate">{{ $t('general.create') }}</button>
      </div>
      <div class='mt-10'></div>
    </div>
  </div>
    ...
    if (!result) {
      ...
      if (verifyExistingAccount) {
        ...
        walletState.wallets.saveMyWalletOnlytoLocalStorage(walletState.currentLoggedInWallet),
          ...
          router.push({ name: "ViewAccountDetails", params: { address: account.address.plain(), accountCreated: "true" } })
      }
    }
  } else {
