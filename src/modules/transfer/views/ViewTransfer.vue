  ...
  <TransactionLayout class="mt-8">
    <template #white>
      ...
      <div class="flex gap-1 mt-3 items-center">
        <SelectInputAccount :type="'transfer'" @select-account="selectAccountAddress"/>
      ...
      <div class="text-blue-primary font-semibold uppercase mt-3 text-xxs">
        Transfer to
      </div>

      <div class="flex mt-1 gap-1">
        <FieldValidationInput
          :placeholder="$t('transfer.transferPlaceholder')"
          v-model="recipientInput"
          v-debounce:1000="checkRecipient"
        ...
        >
          ...
          ></font-awesome-icon>
          <div
            class="text-xxs text-blue-primary font-semibold uppercase ml-auto mr-auto"
          >
            {{ $t("general.select") }}
          </div>
          <div
      ...
      <div v-for="(_, index) in selectedAssets" :key="index">
        ...
        <TransferInputClean
          class="mt-3"
          v-if="selectedAssets[index].id?.length == 16"
          v-model="selectedAssets[index].amount"
          :placeholder="$t('transfer.assetAmount')"
          type="text"
          :decimal="selectedAssets[index].divisibility"
        />
      </div>
      <button
        v-if="selectedAssets.length != assetOptions.length"
      ...
      <TransferInputClean
        ...
        :logo="true"
        type="text"
        @clickedMaxAvailable="updateAmountToMax()"
        :decimal="6"
      />
      ...
      <FieldValidationInput
        ...
        v-debounce:1000="checkPublicKey"
    ...
    <template #navy>
      <TxnSummary
        :signer-native-token-balance="signerNativeTokenBalance"
...
const showBalanceErr = computed(() => {
  if (!selectedAddress.value) {
    ...
    if (
      parseFloat(nativeAmount.value) > nativeTokenBalance.value ||
      !selectedAssets.value.every(
        (asset) => parseFloat(asset.amount) <= asset.balance
      )
    ) {
      return true;
    ...
    if (
      totalFee.value > nativeTokenBalance.value ||
      !selectedAssets.value.every(
        (asset) => parseFloat(asset.amount) <= asset.balance
      )
    ) {
...
const currentBytes = computed(() =>
  message.value.length == 0
    ? 0
    : isEncrypted.value
    ? TransactionUtils.getFakeEncryptedMessageSize(message.value)
    : TransactionUtils.getPlainMessageSize(message.value)
);
...
const maxAmount = computed(() => {
  ...
  if (!selectedMultisigAddress.value) {
    return Helper.convertNumberMinimumFormat(
      nativeTokenBalance.value - txnFee.value,
      tokenDivisibility
    );
  } else {
    return Helper.convertNumberMinimumFormat(
...
const totalFee = computed(() => {
  ...
  if (!selectedMultisigAddress.value) {
    ...
    if (tokenDivisibility == 0) {
      ...
      return (
        Math.round(
          (txnFee.value + lockFundTxFee.value + lockFund.value) *
            Math.pow(10, tokenDivisibility)
        ) / Math.pow(10, tokenDivisibility)
      );
    }
...
const txnFee = computed(() => {
  ...
  return selectedMultisigAddress.value
    ? TransferUtils.calculateAggregateFee(
        message.value,
        nativeAmount.value,
        assets,
        isEncrypted.value
...
const verifyPassword = () => {
  if (isEncrypted.value) {
    let verify = WalletUtils.verifyWalletPassword(
  ...
  if (!verifyResult) {
    ...
    let initiatorAcc: WalletAccount =
      walletState.currentLoggedInWallet.accounts.find(
        (element) => element.address === selectedAddress.value
      );
    ...
    if (isEncrypted.value && message.value.length > 0) {
      ...
      try {
        const accountInfo = await AppState.chainAPI.accountAPI.getAccountInfo(
          Address.createFromRawAddress(recipientInput.value)
        );
    ...
    let transferTransaction = transactionBuilder
      .transferBuilder()
...
const fetchSignerNativeBalance = async () => {
  ...
  try {
    ...
    if (findIndex != -1) {
      signerNativeTokenBalance.value =
...
const checkEncryptable = () => {
  AppState.chainAPI.accountAPI
    .getAccountInfo(Address.createFromRawAddress(recipientInput.value))
    .then((accInfo) => {
...
const checkPublicKey = () => {
  try {
    ...
    if (
      ...
      Address.createFromRawAddress(recipientInput.value).plain()
    ) {
...
const checkRecipient = () => {
  ...
  try {
    ...
    let networkOk = Helper.checkAddressNetwork(
      recipientAddress,
      AppState.networkType
    );
    if (!networkOk) {
