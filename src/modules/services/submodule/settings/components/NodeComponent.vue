  ...
  <div class="mt-5 border border-gray-200 px-2 py-1">
    <div class="h-5 text-left">
      <transition enter-active-class="animate__animated animate__fadeInUp">
        <span v-if="showSelectTitle" class="uppercase text-gray-400 font-light text-txs text-left">{{ placeholder
        }}</span>
      </transition>
    </div>
    <div class="select selectPlugin" style="position: relative">
      <Multiselect :placeholder="placeholder" :options="options" mode="single" :canDeselect="canDeselect"
        v-model="selected" :maxHeight="maxHeight" @select="makeNodeSelection" @close="closeSelection" label="name">
        <template v-slot:singlelabel="{ value }">
          <div class="multiselect-single-label">
            <img src="@/modules/dashboard/img/sirius-icon.svg" class="w-10 h-10 mr-2"> {{ value.name }}
          </div>
        </template>
        <template v-slot:option="{ option }">
          <img src="@/modules/dashboard/img/sirius-icon.svg" class="w-10 h-10 mr-2"> {{ option.name }}
        </template>
      </Multiselect>
    </div>
  </div>
...
const options = computed(() => {
  ...
  let nodeList:{value:string,name:string}[] = [];
...
const makeNodeSelection = (endpoint:string) => {
