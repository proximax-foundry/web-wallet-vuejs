<template>
  <div class="flex justify-between text-xs sm:text-sm">
    <div><span class="text-gray-400">Sirius Gift ></span> <span class="text-blue-primary font-bold">Generate Gift Card</span></div>
    <div>
  ...
  <div class='mt-2 py-3 gray-line lg:px-40'>
    <div v-show="!isGeneratedGift">
      ...
      <div class="mt-10">
        ...
        <button type="submit" class="default-btn py-1 disabled:opacity-50" :disabled="disableGenerate" @click="generateGift()">Generate</button>
      </div>
    </div>
    <div v-show="isGeneratedGift" class="lg:px-40">
      <div class="text-xl font-bold text-blue-primary mt-5">Congratulations!</div>
      <div class="text-sm my-2">Gift card(s) is ready</div>
      ...
      <div class="flex justify-between p-4 rounded-xl bg-white border-yellow-500 border-2 mb-8">
        <div class="text-center w-full">
          ...
          <p class="text-xs mt-3">Make sure you download the file</p>
        </div>
      </div>
      <label class="inline-flex items-center mb-10">
        <input type="checkbox" class="h-5 w-5 bg-blue-primary" value="true" v-model="readCheck">
        <span class="ml-2 cursor-pointer text-sm">I have saved the gift card file.</span>
      </label>
      <div class="mt-10">
        <button type="button" class="default-btn mr-5 focus:outline-none disabled:opacity-50">Save</button>
        <button type="submit" class="default-btn py-1 focus:outline-none disabled:opacity-50" :disabled="!readCheck" @click="backtoPanel()">Continue</button>
      </div>
    </div>
  </div>
</template>
