            ...
            <template #body="{data}">
                ...
                <div>
                    ...
                    <div class="flex items-center  pr-7 ">
                        ...
                        <img v-else-if="data.name[0].name=='prx.xpx'" src="@/modules/account/img/proximax-logo.svg" class='inline-block h-7 w-7 mr-2 border-2 rounded-3xl'>
                        <img v-else-if="data.name[0].name=='xarcade.xar'" src="@/modules/account/img/xarcade-logo.svg" class='inline-block h-7 w-7 mr-2 border-2 rounded-3xl'>
                        <img v-else-if="data.name[0].name=='prx.metx'" src="@/modules/account/img/metx-logo.svg" class='inline-block h-7 w-7 mr-2 border-2 rounded-3xl'>
                        <img v-else  src="@/modules/dashboard/img/icon-proximax-logo-gray.svg" class='inline-block h-6 w-6 mr-2 '>
                        <div v-if="!data.name"></div>
                        <div v-else-if="displayTokenName(data.name[0].name).registered" class="inline-block text-xs ml-2 mt-1 cursor-pointer text-blue-primary"><a :href="explorerNamespaceLink(data.name[0].name)" target=_blank>{{displayTokenName(data.name[0].name).name}}</a></div>
                        <div v-else class="inline-block text-xs ml-2 cursor-pointer text-blue-primary mt-1"><a :href="explorerNamespaceLink(data.name[0].name)" target=_blank>{{data.name[0].name}}</a></div>
                    </div>
                </div>
                <div>
