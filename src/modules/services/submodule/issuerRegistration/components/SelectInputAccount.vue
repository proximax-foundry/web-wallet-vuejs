<template>
    <div>
        <div v-if="!selectedAccountInfo" class="text-blue-primary font-semibold uppercase text-xxs mt-3">Select Account to create / initiate transaction</div>
        <Dropdown :showClear="true"   v-model=selectedAccountInfo :style="{ 'width': '100%' }" :options=accounts :filter="true"
            ...
            @change="selectAccount($event.value?.label, $event.value?.value); $emit('select-account', $event.value?.value);">
                    ...
                    <div class='flex'>
                        ...
                        <div class='flex flex-col ml-2 text-left'>
                            <div class='text-blue-primary font-semibold text-xxs uppercase' style="line-height: 9px;">
                                Selected Account to create / initiate transaction</div>
