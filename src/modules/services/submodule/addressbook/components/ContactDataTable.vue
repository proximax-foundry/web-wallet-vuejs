  ...
  <div>
    ...
    <div class="xl:flex xl:justify-between pb-3 xl:pb-0">
      <div class="md:flex md:items-center md:justify-items-start">
        <SelectInputPluginClean v-model="selectContactGroups" :options="contactGroups" selectDefault=""
          class="w-full md:w-60 mr-4" />
        <div class="w-full md:w-60 px-3 py-1 flex justify-between mb-3 mt-3 md:mt-0 md:mb-0" :class="borderColor">
          <input v-model="filters['global'].value" type="text" class="w-28 outline-none text-xs float-left"
            :placeholder="$t('general.search')" @click="clickInputText()" @blur="blurInputText()">
      ...
      <div class="mt-5 lg:mt-5 xl:mt-0">
        <router-link :to="{ name: 'ViewServicesAddressBookAddContact' }" class="default-btn transition-all duration-300"
          style="padding: auto 3px">+ {{ $t('general.new') }}</router-link>
      </div>
    </div>
    <div class='mt-2 py-3 gray-line'>
      <DataTable :value="formattedContacts" ref="dt" v-model:filters="filters" :paginator="true" :rows="10"
        responsiveLayout="scroll" scrollDirection="horizontal"
        paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown"
        currentPageReportTemplate="" :globalFilterFields="['name', 'address']">
        <Column field="name" headerStyle="width:95%" headerClass="hidden">
          <template #body="{ data }">
            <div class="flex items-center">
              <div v-html="data.svgString" class="mr-2 inline-block"></div>
              <div class="inline-block">
                <div class="text-blue-primary text-tsm">{{ data.name }} <span
                    class="inline-block ml-5 rounded-md text-blue-primary bg-blue-200 px-2 py-1 text-xxs font-bold"
                    v-if="data.group != '-none-'">{{ data.group }}</span></div>
                ...
                <div class="mt-1 text-xs md:text-tsm flex items-center bg-green-100 w-24">
                  ...
                  <font-awesome-icon icon="check" class="w-3 h-3 ml-3 inline-block text-blue-primary"
                    v-if="data.publicKey"></font-awesome-icon>
                  <font-awesome-icon icon="times" class="w-3 h-3 ml-3 inline-block text-blue-primary"
          ...
          <template #body="{ data }">
            <div class="text-txs text-center " @mouseover="hoverOverMenu(data.i)" @mouseout="hoverOutMenu">
              <img src="@/modules/dashboard/img/icon-more-options.svg" class="w-4 h-4 cursor-pointer inline-block"
                @click="showMenu(data.i)">
              <div v-if="isMenuShow[data.i]"
        ...
        <template #empty>
          <div class="text-center my-5"><img src="@/modules/services/submodule/addressbook/img/open-book.svg"
              class="inline-block h-20 w-20"></div>
