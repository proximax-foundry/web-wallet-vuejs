  ...
  <transition enter-active-class="animate__animated animate__fadeInDown"
    ...
    <div v-if="toggleModal" class="popup-outer-lang fixed flex z-50">
      <div class="modal-popup-box text-center">
        <div class='ml-auto mr-auto mt-2 text-xs  font-semibold' style='width:62%'>{{ $t('addressBook.addCustomGroup') }}
        </div>
        <div class="error error_box mb-3" v-if="err != ''">{{ err }}</div>
        <input type="text" v-model="customGroup" :placeholder="$t('addressBook.customGroup')"
        ...
        <button
          ...
          @click="addCustomGroup" :disabled="disableAdd">{{ $t('general.add') }}</button>
        <div class='text-center cursor-pointer font-semibold text-xs text-gray-500 mt-2' @click="closePanel">
