    ...
    export default{
        ...
        setup(p){
            ...
            const fetchNft = async() =>{
                if(p.publicKeys.length == 0){
                    assets.value = []
                }else{
                    assets.value = []
                    for(let j=0;j<p.publicKeys.length;j++){
                        let dataPerRequest = 100
                        let publicAccount = PublicAccount.createFromPublicKey(p.publicKeys[j].publicKey,NetworkType.TEST_NET)
                        ...
                        metadataQueryParams.targetKey = p.publicKeys[j].publicKey
