...
import { NamespaceUtils } from "@/util/namespaceUtils";
    ...
    const buyFromComponent = ref();
    let stableCoins = availableTokens;
    let siriusTokens = ref(availableToTokens);
    const selectedContractAddress = ref("");
    const selectedStableCoins = ref<Array<{ name: string, contractAddress: string, decimals: number }>>([]);
    ...
    const exchangeRate = computed(()=>{
      if(selectedFromTokenPrice.value && selectedToTokenPrice.value){
    ...
    const tokenType = (chainId : number)=>{
    ...
    let ethereumChainId: number
    let bscChainId: number
    let remoteNetworkType: string
    if(networkState.currentNetworkProfile){
      ethereumChainId = networkState.currentNetworkProfile.network.type === NetworkType.MAIN_NET ? 1 : 5;
      bscChainId = networkState.currentNetworkProfile.network.type === NetworkType.MAIN_NET ? 56: 97;
      remoteNetworkType = ethereumChainId === 1 ? "mainnet": "testnet";
    }
    ...
    const searchAccountStableCoinsBalance = async()=>{
      ...
      let contracts : Array<{name:string, contractAddress:string, decimals:number, disabled:boolean }> = [];
    ...
    const checkSelectedTokenSupported = ()=>{
      if(selectedChainId.value === ethereumChainId){
        let coin = ethStableCoins.find(x => x.name === selectedFromToken.value)
        if(coin){
          tokenInvalid.value = coin.disabled;
        }
      }
      else if(selectedChainId.value === bscChainId){
        let coin = bscStableCoins.find(x => x.name === selectedFromToken.value)
        if(coin)
          tokenInvalid.value = coin.disabled;
      }
      else{
    ...
    const updateSelectedContractAddress = ()=>{
      if(selectedStableCoins.value.length){
        let selectedCoins = selectedStableCoins.value.find(x => x.name === selectedFromToken.value)
        if(selectedCoins){
          selectedContractAddress.value = selectedCoins.contractAddress;
        }
      }
    }
    ...
    const handleConnect = (connectData: {chainId: number})=>{
    ...
    const buySiriusToken = async ()=>{
      ...
      try {
        ...
        let selectedCoins = selectedStableCoins.value.find(x => x.contractAddress === selectedContractAddress.value)
        if(selectedCoins){
          const decimals = selectedCoins.decimals;
    ...
    let currentWallet = walletState.currentLoggedInWallet
    if(currentWallet){
      let address = currentWallet.selectDefaultAccount()?.address
      if(address){
        siriusAddress.value = Helper.createAddress(address).pretty();
      }
    }
    checkRecipient();
    ...
    const getSiriusTokenBalance = async() => {
      ...
      let assetIdList: Array<MosaicId> = [];
      let assetAmountList: Array<UInt64> = [];
      ...
      const filteredMosaics = mosaicInfo.reduce((filtered: Array<{names: string[], id:MosaicId, amount:number}>, mosaic) => {
        if(mosaic.names.length > 0){
          let amount = assetAmountList[assetIdList.findIndex(asset => asset.toHex() == mosaic.mosaicId.toHex())];
          let nsNames: Array<string> = [];
    ...
    const saveCertificate = async () => {
      ...
      SwapUtils.generateIncomingPdfCert(selectedChainId.value === bscChainId ? "BSC" : "ETH", swapTimestamp.value, siriusAddress.value, selectedFromToken.value, transactionHash.value, await swapQr);
    };
