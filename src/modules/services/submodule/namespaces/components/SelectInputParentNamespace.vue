<template>
  <div class="relative" :class="disabled ? 'opacity-50' : 'cursor-pointer'">
    <div @click='(!disabled) ? toggleSelection = !toggleSelection : ""'
      class="border ml-auto mr-auto py-3 px-2  rounded-md w-full h-14">
      <div class='flex'>
        ...
        <img v-if='toggleSelection' @click='selectedNamespace = ""; $emit("clear-namespace");' src="@/assets/img/delete.svg"
    ...
    <div class='relative'>
      <div v-if='toggleSelection'
        ...
        <div v-if='namespaces.length > 0' class="uppercase pl-2 pt-4 text-xxs text-gray-400">
          {{ $t('namespace.selectNamespace') }}</div>
        <div v-else class='text-xxs pt-2 pl-2 pb-2'>{{ $t('general.listEmpty') }}</div>
        <div v-for='(item, index) in namespaces' :key="index" class="px-2 py-3 flex items-center" @click="selectNs(item)"
          :class='[(index != namespaces.length - 1) ? "border-b border-gray-200" : "", (!item.disabled) ? "cursor-pointer" : "cursor-text"]'>
...
const selectNamespace = (ns :{value:string,level:string[],label:string,disabled:boolean}) => {
...
const namespaces = computed(() => {
  let namespace :{value:string,level:string[],label:string,disabled:boolean}[] = [];
  namespace.push({
    value: '1',
...
const selectNs = (item :{value:string,level:string[],label:string,disabled:boolean}) => {
  if (!item.disabled) {
