<template>
  <div class="flex justify-between text-md">
    <div><span class="text-gray-300">{{$t('common.namespaces')}} ></span> <span class="text-blue-primary font-bold">{{$t('common.extend')}}</span></div>
    <div>
    ...
    <div class="md:col-span-3">
      <form @submit.prevent="extendNamespace">
        <fieldset class="w-full">
          <div class="mb-5">
            ...
            <div v-if="getMultiSigCosigner.list.length > 0">
              <div class="text-tsm">{{$t('common.cosigner')}}:
                <span class="font-bold" v-if="getMultiSigCosigner.list.length == 1">{{ getMultiSigCosigner.list[0].name }} ({{$t('services.balance')}}: {{ getMultiSigCosigner.list[0].balance }} XPX) <span v-if="getMultiSigCosigner.list[0].balance < lockFundTotalFee" class="error">- {{$t('common.insufficientBalance')}}</span></span>
          ...
          <div class="rounded-2xl bg-gray-100 p-5 mb-5">
            <div class="inline-block mr-4 text-xs"><img src="@/assets/img/icon-prx-xpx-blue.svg" class="w-5 inline mr-1 text-gray-500">{{$t('common.rentalFee')}}: {{ rentalFee }} {{currencyName}}</div>
          </div>
          ...
          <div class="mt-10">
            ...
            <button type="submit" class="default-btn py-1 disabled:opacity-50" :disabled="disableCreate" @click="extendNamespace()">{{$t('common.create')}}</button>
          </div>
        </fieldset>
      </form>
    </div>
