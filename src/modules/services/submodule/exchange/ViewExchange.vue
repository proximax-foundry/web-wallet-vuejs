  ...
  <TransactionLayout>
    <template #white>
      ...
      <div>
        {{
          Helper.toCurrencyFormat(
            offerInfo?.currentMosaicGiveAmount.compact() /
              Math.pow(10, offerInfo?.mosaicGiveDivisibility),
            offerInfo?.mosaicGiveDivisibility
          )
        }}
        {{ displayAssetName(offerInfo?.mosaicGiveName) }}
      </div>
      ...
      <div>
        {{
          Helper.toCurrencyFormat(
            baseRatio[0] / Math.pow(10, offerInfo?.mosaicGetDivisibility),
            offerInfo?.mosaicGetDivisibility
          )
        }}
        {{ displayAssetName(offerInfo?.mosaicGetName) }} :
        {{
          Helper.toCurrencyFormat(
            baseRatio[1] / Math.pow(10, offerInfo?.mosaicGiveDivisibility),
            offerInfo?.mosaicGiveDivisibility
          )
        }}
        {{ displayAssetName(offerInfo?.mosaicGiveName) }}
      ...
      <div class="text-blue-primary mt-3">
        Amount to give ({{ displayAssetName(offerInfo?.mosaicGetName) }})
      </div>
      ...
      <div
        ...
        v-if="parseFloat(amountToPay) > giveAssetBalance"
      >
      ...
      <div
        ...
        "
      >
        Less than minimum offer amount
      </div>
      ...
      <div class="flex mt-2">
        ...
        <div
          ...
          v-if="
            Math.trunc(
              amountToGet * Math.pow(10, offerInfo?.mosaicGiveDivisibility)
            ) /
              Math.pow(10, offerInfo?.mosaicGiveDivisibility) >
            offerInfo?.currentMosaicGiveAmount.compact() /
              Math.pow(10, offerInfo?.mosaicGiveDivisibility)
      ...
      <div
        ...
        v-if="
          Math.trunc(
            exceededAmount * Math.pow(10, offerInfo?.mosaicGetDivisibility)
          ) /
            Math.pow(10, offerInfo?.mosaicGetDivisibility) >
          0
        "
      >
        {{
        ...
        {{ displayAssetName(offerInfo?.mosaicGetName) }} will be ignored
      </div>
      ...
      >
        {{
          Helper.toCurrencyFormat(
            parseFloat(amountToPay) - exceededAmount,
            offerInfo?.mosaicGetDivisibility
          )
        }}
        {{ displayAssetName(offerInfo?.mosaicGetName) }} :
      ...
      <div v-else>
        0 {{ displayAssetName(offerInfo?.mosaicGetName) }} : 0
        {{ displayAssetName(offerInfo?.mosaicGiveName) }}
      </div>
    </template>
...
const exceededAmount = computed(() => {
  if (
    parseFloat(amountToPay.value) <
    baseRatio.value[0] / Math.pow(10, offerInfo.value?.mosaicGetDivisibility)
  ) {
    return 0;
  }
  return (
    ((parseFloat(amountToPay.value) *
      Math.pow(10, offerInfo.value?.mosaicGetDivisibility)) %
      baseRatio.value[0]) /
    Math.pow(10, offerInfo.value?.mosaicGetDivisibility)
  );
});
...
const amountToGet = computed(() => {
  return (
    parseFloat(amountToPay.value) * rate.value -
    exceededAmount.value * rate.value
  );
});
  ...
  const sdaExchangeOfferTxn = PlaceSdaExchangeOfferTransaction.create(
    ...
    [
      new SdaExchangeOffer(
        ...
        UInt64.fromUint(
          parseFloat(amountToPay.value) *
            Math.pow(10, offerInfo.value?.mosaicGetDivisibility ?? 0)
        ),
        new MosaicId(AppState.nativeToken.assetId),
        UInt64.fromUint(
          amountToGet.value *
            Math.pow(10, offerInfo.value?.mosaicGiveDivisibility ?? 0)
        ),
...
const totalFee = computed(() => {
  ...
  if (
    offerInfo.value &&
    offerInfo.value.mosaicIdGet.id.toHex().toLowerCase() ==
      AppState.nativeToken.assetId.toLowerCase()
  ) {
    return (
      Math.round(
        (parseFloat(amountToPay.value) - exceededAmount.value + txnFee.value) *
          Math.pow(10, AppState.nativeToken.divisibility)
      ) / Math.pow(10, AppState.nativeToken.divisibility)
    );
  }
...
const baseRatio = computed(() => {
  ...
  const numerator =
    offerInfo.value.initialMosaicGiveAmount.compact(); /* / Math.pow(10, offerInfo.value.mosaicGiveDivisibility ?? 0) * Math.pow(10, 6) */
  const denominator =
    offerInfo.value.initialMosaicGetAmount.compact(); /* / Math.pow(10, offerInfo.value.mosaicGetDivisibility ?? 0) * Math.pow(10, 6) */
...
watch(
  ...
  (n) => {
    if (n.mosaicGetDivisibility) {
      amountToPay.value = (
        baseRatio.value[0] / Math.pow(10, n.mosaicGetDivisibility)
      ).toString();
    }
  },
  { deep: true }
...
const fetchExchange = async () => {
  ...
  const accountExchangeOffers = await lastValueFrom(
    sdaExchangeHttp.getAccountSdaExchangeOffers(
      Address.createFromRawAddress(props.owner)
    )
  );
  offerInfo.value = accountExchangeOffers.sdaOfferBalances.find(
    (offer) =>
      offer.mosaicIdGive.toHex() == giveAsset &&
      offer.mosaicIdGet.toHex() == getAsset
  );
  if (!offerInfo.value) {
    toast.add({
      severity: "error",
  ...
  const namespaceInfo = await AppState.chainAPI.assetAPI.getMosaicsNames([
    offerInfo.value.mosaicIdGet as MosaicId,
    offerInfo.value.mosaicIdGive as MosaicId,
  ]);
  const assetInfo = await AppState.chainAPI.assetAPI.getMosaics([
    offerInfo.value.mosaicIdGet as MosaicId,
    offerInfo.value.mosaicIdGive as MosaicId,
  ]);

  offerInfo.value.mosaicGetName = namespaceInfo[0].names.length
    ? namespaceInfo[0].names[0].name
    : offerInfo.value.mosaicIdGet.toHex();
  offerInfo.value.mosaicGiveName = namespaceInfo[1].names.length
    ? namespaceInfo[1].names[0].name
    : offerInfo.value.mosaicIdGive.toHex();
  offerInfo.value.mosaicGetDivisibility = assetInfo.find(
    (info) => info.mosaicId.toHex() == offerInfo.value.mosaicIdGet.toHex()
  ).divisibility;
  offerInfo.value.mosaicGiveDivisibility = assetInfo.find(
    (info) => info.mosaicId.toHex() == offerInfo.value.mosaicIdGive.toHex()
  ).divisibility;
  rate.value =
    offerInfo.value.initialMosaicGiveAmount.compact() /
    Math.pow(10, offerInfo.value.mosaicGiveDivisibility) /
    (offerInfo.value.initialMosaicGetAmount.compact() /
      Math.pow(10, offerInfo.value.mosaicGetDivisibility));
};
...
const fetchGiveAssetBalance = async (address: string) => {
  try {
    ...
    const findAsset = accountInfo.mosaics.find(
      (asset) => asset.id.toHex() == offerInfo.value.mosaicIdGet.toHex()
    );
    if (!findAsset) {
      giveAssetBalance.value = 0;
      return;
    }
    giveAssetBalance.value =
      findAsset.amount.compact() /
      Math.pow(10, offerInfo.value.mosaicGetDivisibility);
  } catch (error) {
    giveAssetBalance.value = 0;
  }
...
const exchangeSell = async () => {
  ...
  if (
    parseFloat(amountToPay.value) - exceededAmount.value >
    giveAssetBalance.value
  ) {
    toast.add({
      severity: "error",
  ...
  if (
    Math.trunc(
      amountToGet.value * Math.pow(10, offerInfo.value.mosaicGiveDivisibility)
    ) /
      Math.pow(10, offerInfo.value.mosaicGiveDivisibility) >
    offerInfo.value.currentMosaicGiveAmount.compact() /
      Math.pow(10, offerInfo.value.mosaicGiveDivisibility)
  ) {
    toast.add({
      severity: "error",
      ...
      detail:
        "Exceed available amount of " +
        displayAssetName(offerInfo.value.mosaicGiveName),
      group: "br-custom",
      life: 1000,
    });
    return;
