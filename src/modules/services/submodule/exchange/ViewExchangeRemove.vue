<template>
  <TransactionLayout class="mt-8">
    <template #white>
      ...
      <div v-if="isMultisig && !cosignerList.length" class="text-red-500">
        Cosigner not found
      </div>
      ...
      <div>
        {{
          offerInfo?.currentMosaicGetAmount.compact() /
    ...
    <template #navy>
      <div v-if="isMultisig">
        ...
        <div class="lg:grid lg:grid-cols-5 grid grid-cols-7 text-gray-200 my-1">
          ...
          <span
            ...
            v-html="getNativeTokenBalance(owner)"
          ></span>
          <div class="flex">
      ...
      <div class="lg:grid lg:grid-cols-5 grid grid-cols-7 text-gray-200 my-1">
        ...
        <span
          class="ml-auto lg:col-span-2 col-span-3"
          v-html="
            getNativeTokenBalance(isMultisig ? selectedCosignerAddress : owner)
          "
        ></span>
        <div class="flex">
          <div class="ml-1 text-blue-400 font-bold">
...
const accountName = computed(() => {
  ...
  return walletState.currentLoggedInWallet.convertAddressToName(
    plainAddress,
    true
  );
});
...
const txnFee = computed(() => {
  ...
  if (!isMultisig.value) {
    return (
      removeSdaExchangeTxn.maxFee?.compact() /
  ...
  const multisigAcc = [
    ...
    (acc) => acc.address == Address.createFromRawAddress(props.owner).plain()
  );
  const innerTxn = [
    removeSdaExchangeTxn.toAggregateV1(
...
const removeOffer = async () => {
  ...
  const initiatorAcc = walletState.currentLoggedInWallet.accounts.find(
    (acc) =>
      acc.address ==
      (isMultisig.value ? selectedCosignerAddress.value : plainAddress)
  );
  ...
  if (!isMultisig.value) {
    ...
    const multisigAcc = [
      ...walletState.currentLoggedInWallet.accounts,
      ...walletState.currentLoggedInWallet.others,
    ].find((acc) => acc.address == plainAddress);
    const innerTxn = [
      removeSdaExchangeTxn.toAggregateV1(
        PublicAccount.createFromPublicKey(
          multisigAcc.publicKey,
