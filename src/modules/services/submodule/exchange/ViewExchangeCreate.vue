            ...
            <div class='border-2 xl:grid xl:grid-cols-3'>
                <div class="xl:col-span-2 p-12">
                    ...
                    <div v-if="!selectedAddress" class="text-xs mt-3 text-blue-primary ">Select Account to Create / Initiate
                        Transaction</div>
                    <SelectInputAccount :label="'Create SDA-SDA Exchange'"/>
                ...
                <div class='bg-navy-primary py-6 px-6 xl:col-span-1'>
                    ...
                    <div v-if="selectedMultisigAddress">
                        ...
                        <div v-if="assetToGive"
                            class="lg:grid lg:grid-cols-5 grid grid-cols-7 text-gray-200 my-1 items-center">
                            <div class='font-semibold text-xxs mt-2 lg:col-span-2 col-span-3 text-blue-primary uppercase'>
                                Asset to Give</div>
                            <span class='ml-auto lg:col-span-2 col-span-3'> {{ amountToGive }}</span>
                            <span class="ml-1 text-blue-400 text-xs">{{ displayAssetName(assetToGive.namespace.length ?
                                assetToGive.namespace : assetToGive.id) }}</span>
                    ...
                    <div class="lg:grid lg:grid-cols-5 grid grid-cols-7 text-gray-200 my-1">
                        <div class='font-semibold text-xxs mt-2 lg:col-span-2 col-span-3 text-blue-primary uppercase'>
                            {{ $t('general.currentBalance') }}</div>
                        <span class='ml-auto lg:col-span-2 col-span-3'
                            v-html="getNativeTokenBalance(selectedAddress)"></span>
                        <div class="flex">
                            <div class='ml-1 text-blue-400 font-bold'>{{ AppState.nativeToken.label }}</div>
                            <img src="@/modules/account/img/proximax-logo.svg" class='ml-1 h-5 w-5 mt-0.5'>
                        </div>
                    ...
                    <div v-if="!selectedMultisigAddress">
                        <div v-if="assetToGive"
                            ...
                            <div class='font-semibold text-xxs mt-2 lg:col-span-2 col-span-3 text-blue-primary uppercase'>
                                Asset to Give</div>
                            <span class='ml-auto lg:col-span-2 col-span-3 font-semibold'> {{ amountToGive }}</span>
                            <span class="ml-1 text-blue-400 text-xs">{{ displayAssetName(assetToGive.namespace.length ?
                                assetToGive.namespace : assetToGive.id) }}</span>
...
const assetToReceiveProperties = ref<{ supply: number, divisibility: number, namespace: string } | null>(null)
...
const checkAsset = async () => {
    try {
        ...
        assetToReceiveProperties.value = { supply: properties.supply.compact(), divisibility: properties.divisibility, namespace: namespaceName.length ? namespaceName[0].names[0].name : mosaicId.toHex() }
    ...
    try {
        ...
        assetToReceiveProperties.value = { supply: properties.supply.compact(), divisibility: properties.divisibility, namespace: namespaceName[0]?.names[0]?.name ?? id.toHex() }
        assetValid.value = true;
    } catch (_) {
        assetValid.value = false;
        assetToReceiveProperties.value = null;
