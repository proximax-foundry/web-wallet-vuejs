<template>
  <div class="flex justify-between text-xs sm:text-sm">
    <div><span class="text-gray-400">{{$t('common.vote')}} ></span> <span class="text-blue-primary font-bold">{{$t('vote.voteResults')}}</span></div>
    <div>
      <router-link :to="{ name: 'ViewServicesVotingPoll'}" class="font-bold">{{$t('common.viewAll')}}</router-link> |
  ...
  <div class='mt-2 py-3 gray-line'>
    <div class='mt-2 py-3 text-center px-0 lg:px-10 xl:px-80'>
      ...
      <div class=" border-t-4 border-b-4 border-solid border-gray-300 mt-5 py-2">
        <div><b>[Status]</b> <div class="inline-block mx-2">></div> {{$t('common.finish')}} <div class="inline-block mx-2">></div> [date] </div>
      </div>
          ...
          <div class="grid grid-cols-4 h-full">
            ...
            <div class="col-span-3 text-left pl-2 flex">
              <div class="self-center">
                <b class="text-sm">{{$t('common.hash')}}:</b>
      ...
      <div class="mt-20">
        <button type="button" class="default-btn mr-5 focus:outline-none disabled:opacity-50" :disabled="voted || voteEnded">{{$t('common.vote')}}</button>
        <button type="button" class="default-btn mr-5 focus:outline-none disabled:opacity-50" @click="showCertificateModel=true" :disabled="!voted">{{$t('common.certificate')}}</button>
        <button type="submit" class="default-btn py-1 focus:outline-none disabled:opacity-50" @click="showResultModel=true" :disabled="!(voteEnded || (!voteEnded && voted))">{{$t('common.result')}}</button>
      </div>
    </div>
