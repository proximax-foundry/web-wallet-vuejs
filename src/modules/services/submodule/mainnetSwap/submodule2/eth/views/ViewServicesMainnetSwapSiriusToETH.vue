  ...
  <div>
    ...
    <div class='mt-6 p-6 border filter shadow-lg text-center'>
      ...
      <div v-if="currentPage==1">
        ...
        <SelectInputAccountOutgoingSwap v-model="siriusAddress" placeholder="From Sirius Chain Account" :selectDefault="walletState.currentLoggedInWallet?.selectDefaultAccount()?.address as string" />
        <div class="relative">
...
//import { listenerState } from "@/state/listenerState";
    ...
    const callSwapServer = async(payload:string) =>{
      ...
      try {
        ...
        if(response.ok){
          ...
          SwapUtils.generateQRCode(ethScanUrl + res.data.txHash).then((x)=>{
          swapQr.value = x
        });
          currentPage.value = 2;
        }
    ...
    const callTocheckSwapStatus =  async() =>{
      ...
      if(response.status==200){
        ...
        SwapUtils.generateQRCode(ethScanUrl + res.fulfillTransaction).then((x)=>{
          swapQr.value = x
        });
        currentPage.value = 2;
      }
