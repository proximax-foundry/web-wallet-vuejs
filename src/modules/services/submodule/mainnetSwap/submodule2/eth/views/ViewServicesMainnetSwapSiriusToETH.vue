<template>
  <div class="flex justify-between text-xs sm:text-sm">
    <div><span class="text-gray-400">{{$t('swap.swap')}} > {{$t('services.eth')}} > {{$t('swap.outcome')}} > </span> <span class="text-blue-primary font-bold">{{$t('swap.transaction')}}</span></div>
    <div>
  ...
  <div class='mt-2 py-3 gray-line px-0 lg:px-10 xl:px-80'>
    ...
    <div v-if="currentPage==2">
      ...
      <div class="tex-center font-bold text-lg mb-2">{{$t('swap.ethtxfee')}}:</div>
      <div class="md:grid md:grid-cols-3 mb-10">
        ...
        <div class="md:col-span-1 mb-3">
          <div class="ethNetwork md:ml-6" :class="`${ (ethNetwork == 'trader')?'selected':'option' }`" @click="ethNetwork='trader'">
            <p class="font-bold text-tsm">{{$t('swap.trader')}}</p>
    ...
    <div v-if="currentPage==3">
      <div>
        ...
        <swap-certificate-component networkTerm="ETH" swapType="Outcome" />
        <div class="flex justify-between p-4 rounded-xl bg-white border-yellow-500 border-2 my-8">
          <div class="text-center w-full">
            <div class="w-8 h-8 inline-block relative">
              <div class="rounded-full border border-yellow-500 w-7 h-7 relative">
