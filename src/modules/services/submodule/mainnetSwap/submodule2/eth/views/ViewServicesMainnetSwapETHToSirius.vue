<template>
  ...
  <div class='mt-2 py-3 gray-line px-0 lg:px-10 xl:px-80'>
    <div class="flex">
      ...
      <div class="flex-none">
        <div class="flex p-0 sm:p-3">
          ...
          <div class="inline-block self-center ml-3 text-xs sm:text-sm">{{$t('common.validation')}}</div>
        </div>
      </div>
      <div class="h-1 bg-gray-200 flex-grow mx-2 self-center"></div>
      <div class="flex-none">
    ...
    <div v-if="currentPage==1">
      ...
      <SupplyInput :disabled="disableAmount" v-model="amount" :balance="balance" title="eXPX (Minimum 51 eXPX required)" placeholder="eXPX" type="text" icon="coins" :showError="showAmountErr" :errorMessage="(!amount)?'Required Field':((parseFloat(amount) <= defaultXPXTxFee)?'Insufficient amount':'Insufficient token balance.')" :decimal="6" />
    ...
    <div v-if="currentPage==2">
      ...
      <div class="mt-10">
        <button type="submit" class="default-btn focus:outline-none disabled:opacity-50" :disabled="isDisabledValidate" @click="validated()">{{$t('common.continue')}}</button>
      </div>
    </div>
