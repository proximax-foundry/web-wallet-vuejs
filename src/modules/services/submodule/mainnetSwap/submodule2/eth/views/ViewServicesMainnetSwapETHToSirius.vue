      ...
      <div class='mt-6 p-6 border filter shadow-lg text-center'>
        ...
        <div v-if="currentPage==2">
          <div class="text-lg my-7">
            <div class="error error_box mb-5" v-if="err!=''">{{ err }}</div>
            <div class="font-bold text-left text-xs md:text-sm" :class="step1?'text-gray-700':'text-gray-300'">Step 1: Send ERC20 {{currentNativeTokenName}} to the escrow account</div>
        ...
        <div v-if="currentPage==3">
          <div>
            ...
            <swap-certificate-component networkTerm="ETH" swapType="In" :swapId="swapId" :swapTimestamp="swapTimestamp" :transactionHash="transactionHash" :siriusName="siriusName" :swappedAmount="Number(amount)" :siriusAddress="Helper.createAddress(siriusAddress as string).pretty()" :swapQr="swapQr" :swapLink="validationLink" />
...
import type { Account } from "@/models/account";
    ...
    const swapServerUrl = SwapUtils.getIncoming_ETHSwapTransfer_URL(swapData.swap_IN_SERVICE_URL, "xpx");
    ...
    const saveCertificate = () => {
      SwapUtils.generateIncomingPdfCert('ETH', swapTimestamp.value, siriusAddress.value as string, swapId.value, transactionHash.value, swapQr.value);
    };
    ...
    const isDisabledSwap = computed(() =>
      ...
      !(Number(amount.value) > 0 && siriusAddress.value != '' && !err.value && (balance.value >= Number(amount.value)) && (Number(amount.value) > defaultXPXTxFee.value))
    );
    const amount = ref('0');

    const siriusName = computed(() => {
      let accountSelected = walletState.currentLoggedInWallet?.accounts.find(account => account.address == siriusAddress.value) as Account;
      if(!accountSelected){
        accountSelected = walletState.currentLoggedInWallet?.others.find(account => account.address == siriusAddress.value) as Account;
      }
      return accountSelected.name;
    });

    const amountReceived = computed(() => {
      if((Number(amount.value) - 50) >0 ){
        return (Number(amount.value) - 50);
      }else{
        return 0;
      }
    });
    ...
    watch(amount, (n) => {
      if(Number(n) <= defaultXPXTxFee.value){
        showAmountErr.value = true;
      }else{
        showAmountErr.value = false;
      }
