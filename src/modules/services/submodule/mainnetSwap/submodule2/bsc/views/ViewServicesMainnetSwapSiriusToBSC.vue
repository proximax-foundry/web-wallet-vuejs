<template>
  <div class="flex justify-between text-sm">
    <div><span class="text-gray-400">{{$t('swap.swap')}}> {{$t('services.bsc')}} >{{$t('swap.outcome')}} ></span> <span class="text-blue-primary font-bold">{{$t('swap.transaction')}}</span></div>
    <div>
  ...
  <div class='mt-2 py-3 gray-line px-0 lg:px-10 xl:px-80'>
    ...
    <div v-if="currentPage==1">
      <p class="text-tsm my-5 text-gray-400">{{$t('swap.siriusaccountlist')}}.</p>
      <div class="text-lg my-7 font-bold">{{$t('swap.selectsiriusaccount')}}</div>
      <div class="mb-2 flex justify-between bg-gray-100 rounded-2xl p-3 text-left cursor-pointer hover:bg-blue-100 transition" @click="selectAccount">
        <div class="text-tsm ml-3 text-gray-700">
          ...
          <div><b>{{$t('swap.siriusaddress')}}:</b> [[account address]]</div>
          <div><b>{{$t('swap.siriusbalance')}}:</b> <img src="@/assets/img/icon-prx-xpx-blue.svg" class="w-5 inline ml-1"> [[balance]] XPX</div>
        </div>
    ...
    <div v-if="currentPage==2">
      ...
      <div class="tex-center font-bold text-lg mb-2">{{$t('swap.bsctxfee')}}:</div>
      <div class="rounded-2xl bg-gray-100 p-5 mb-5">
        <div class="inline-block mr-4 text-xs"><img src="@/assets/img/icon-prx-xpx-blue.svg" class="w-5 inline mr-1 text-gray-500">{{$t('namespace.transactionfee')}} BNB 0.051000 &#8776; XPX 0.051000 &#8776; USD 0.08</div>
      </div>
      <div class="tex-center font-bold text-lg mb-2">{{$t('namespace.transactionfee')}}</div>
      <div class="rounded-2xl bg-gray-100 p-5 mb-5">
      ...
      <div class="mt-10">
        ...
        <button type="submit" class="default-btn focus:outline-none disabled:opacity-50" :disabled="isDisabledSwap" @click="swap">{{$t('swap.confirmswap')}}</button>
      </div>
    </div>
    <div v-if="currentPage==3">
      <div>
        ...
        <swap-certificate-component networkTerm="BSC" swapType="Outcome" />
        <div class="flex justify-between p-4 rounded-xl bg-white border-yellow-500 border-2 my-8">
          <div class="text-center w-full">
            <div class="w-8 h-8 inline-block relative">
              <div class="rounded-full border border-yellow-500 w-7 h-7 relative">
        ...
        <div class="mt-10">
          <button type="button" class="hover:shadow-lg bg-white hover:bg-gray-100 rounded-3xl border-2 font-bold px-6 py-2 border-blue-primary text-blue-primary outline-none focus:outline-none mr-4 w-32">{{$t('accounts.save')}}</button>
