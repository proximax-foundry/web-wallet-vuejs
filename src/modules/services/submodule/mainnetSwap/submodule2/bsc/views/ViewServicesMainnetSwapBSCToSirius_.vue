  ...
  <div>
    ...
    <div class='mt-2 py-3 gray-line px-0 lg:px-10 xl:px-80'>
      ...
      <div v-if="currentPage==1">
        ...
        <SupplyInput :disabled="disableAmount" v-model="amount" :balance="balance" :title="'BEP20 ' + currentNativeTokenName + ' (minimum = 51)'" :placeholder="'BEP20 ' + currentNativeTokenName" type="text" icon="coins" :showError="showAmountErr" :errorMessage="(!amount)?'Required Field':((parseFloat(amount) <= defaultXPXTxFee)?'Insufficient amount':'Insufficient token balance.')" :decimal="6" />
      ...
      <div v-if="currentPage==2">
        <div class="text-lg my-7">
          <div class="error error_box mb-5" v-if="err!=''">{{ err }}</div>
          <div class="font-bold text-left text-xs md:text-sm lg:text-lg" :class="step1?'text-gray-700':'text-gray-300'">Step 1: Send BEP20 {{currentNativeTokenName}} to the escrow account</div>
