<template>
  <div class="flex justify-between text-xs sm:text-sm">
    <div><span class="text-gray-400">{{$t('swap.swap')}} > {{$t('services.bsc')}} > </span> <span class="text-blue-primary font-bold">{{$t('swap.selectoption')}}</span></div>
    <div>
      ...
      <div class="md:col-span-1">
        <router-link :to="{ name: 'ViewServicesMainnetSwapSiriusToBSC' }">
          <div class="m-5 lg:mx-10 rounded-2xl border border-blue-primary option-div">
            ...
            <div class="mt-3 mb-5 sm:mb-10 text-gray-500">{{$t('swap.sirius2bsc')}}</div>
          </div>
        </router-link>
      </div>
      <div class="md:col-span-1">
        <router-link :to="{ name: 'ViewServicesMainnetSwapBSCToSirius' }">
          <div class="m-5 lg:mx-10 rounded-2xl border border-blue-primary option-div">
            ...
            <div class="mt-3 mb-5 sm:mb-10 text-gray-500">{{$t('swap.bsc2sirius')}}</div>
          </div>
        </router-link>
      </div>
    </div>
