    ...
    <div class='lg:w-9/12 ml-2 mr-2 lg:ml-auto lg:mr-auto mt-5'>
      <div class='mt-6 p-6 border filter shadow-lg text-center'>
        ...
        <div v-if="currentPage==1">
          ...
          <SupplyInputClean :disabled="disableAmount" v-model="amount" :balance="balance" :placeholder="'BEP20 ' + `${selectedToken?selectedToken.name:''}`" type="text" :showError="showAmountErr" :errorMessage="(!amount)?'Required Field':Number(amount)>=minAmount?$t('swap.insufficientTokenBalance'):`Min. amount is ${minAmount}(${feeAmount} ${selectedToken?.name.toUpperCase()} will deducted for transaction fee)`" :divisibility="tokenDivisibility"  />
          <div class="flex">
        ...
        <div v-if="currentPage==3">
          <div>
            ...
            <swap-certificate-component :networkTerm="$t('swap.bsc')" swapType="In" :swapToken="swapToken" :swapTimestamp="swapTimestamp" :transactionHash="transactionHash" :siriusName="siriusName" :swappedAmount="Number(amount)" :siriusAddress="Helper.createAddress(siriusAddress).pretty()" :swapQr="swapQr" :swapLink="validationLink" />
...
import {Address, NamespaceId} from 'tsjs-xpx-chain-sdk';
  ...
  const accounts = computed<{name:string; balance:number; address:string; publicKey:string; isMultisig:boolean;}[]>(()=>{
   ...
   const contacts = computed(() => {
    ...
    const selectedToken = ref<{name:string}>();
    ...
    function handleAccountsChanged(accounts:{name:string; balance:number; address:string; publicKey:string; isMultisig:boolean;}[]) {
      if(window.ethereum.isMetaMask){
        if (accounts.length === 0) {
          // MetaMask is locked or the user has not connected any accounts
          // console.log('Please connect to MetaMask.');
