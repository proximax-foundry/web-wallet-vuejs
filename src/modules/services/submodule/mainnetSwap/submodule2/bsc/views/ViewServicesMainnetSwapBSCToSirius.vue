  ...
  <div>
    <div class='lg:w-9/12 ml-2 mr-2 lg:ml-auto lg:mr-auto mt-5'>
      <div class='mt-6 p-6 border filter shadow-lg text-center'>
        ...
        <div class="text-xs my-3 mb-5 sm:mb-10"><img src="@/modules/services/submodule/mainnetSwap/img/bsc.svg"
            class="mr-2 h-5 inline-block">{{ $t('swap.swapBscToSirius') }}</div>
        <div class="flex my-10">
          <div class="flex-none">
            <div class="flex border border-gray-300 rounded-md filter shadow-md">
              <div class="flex w-6 h-6 sm:w-10 sm:h-10" :class="`${currentPage >= 1 ? 'bg-yellow-500' : 'bg-gray-300'}`">
                <div class="self-center inline-block text-center w-full text-txs sm:text-sm font-bold"
                  :class="`${currentPage >= 1 ? 'text-white' : 'text-gray-400'}`">1</div>
              </div>
              <div class="px-4 sm:px-10 self-center text-xxs sm:text-xs hidden md:inline-block lg:hidden xl:inline-block">
                {{ $t('general.transaction') }}</div>
            </div>
          </div>
          <div class="flex-grow self-center md:mx-4 h-0.5 bg-gray-100"></div>
          <div class="flex-none">
            <div class="flex border border-gray-300 rounded-md filter shadow-md">
              <div class="flex w-6 h-6 sm:w-10 sm:h-10" :class="`${currentPage >= 2 ? 'bg-yellow-500' : 'bg-gray-300'}`">
                <div class="self-center inline-block text-center w-full text-txs sm:text-sm font-bold"
                  :class="`${currentPage >= 2 ? 'text-white' : 'text-gray-400'}`">2</div>
              </div>
              <div class="px-4 sm:px-10 self-center text-xxs sm:text-xs hidden md:inline-block lg:hidden xl:inline-block">
                {{ $t('swap.validation') }}</div>
            </div>
          </div>
          <div class="flex-grow self-center md:mx-4 h-0.5 bg-gray-100"></div>
          <div class="flex-none">
            <div class="flex border border-gray-300 rounded-md filter shadow-md">
              <div class="flex w-6 h-6 sm:w-10 sm:h-10" :class="`${currentPage == 3 ? 'bg-yellow-500' : 'bg-gray-300'}`">
                <div class="self-center inline-block text-center w-full text-txs sm:text-sm font-bold"
                  :class="`${currentPage >= 3 ? 'text-white' : 'text-gray-400'}`">3</div>
              </div>
              <div class="px-4 sm:px-10 self-center text-xxs sm:text-xs hidden md:inline-block lg:hidden xl:inline-block">
                {{ $t('general.certificate') }}</div>
            </div>
          </div>
        </div>
        <div v-if="currentPage == 1">
          ...
          <button @click="recheckMetamask()" v-if="!isInstallMetamask" class="text-xs blue-btn p-2 mb-3">Recheck
            MetaMask</button>
          <div class="bg-yellow-200 text-yellow-900 text-tsm p-3 mb-5 rounded-2xl" v-if="!verifyMetaMaskPlugin">
            {{ $t('swap.noOtherExtension') }} <b>{{ $t('swap.metamask') }}</b>.<div class="my-2">{{ $t('swap.referTo') }}<a
                href="https://bit.ly/3mVayCu" target=_new
                class="text-blue-primary">{{ $t('swap.walkthrough') }}<font-awesome-icon icon="external-link-alt"
                  class="text-blue-primary w-3 h-3 self-center inline-block ml-1"></font-awesome-icon></a>{{ $t('swap.forMoreDetails') }}
            </div>{{ $t('swap.refreshMsg') }}</div>
          ...
          <div class="mb-5 md:flex md:justify-between border border-gray-200 rounded">
            <div class="flex justify-left">
              <div class="w-18 flex items-center justify-center py-5 sm:h-20">
                <img src="@/modules/services/submodule/mainnetSwap/img/icon-metamask-fox.svg"
                  class="w-8 h-8 inline-block">
              </div>
              <div class="text-left flex items-center">
                <div>
                  <div class="text-xxs uppercase text-blue-primary font-bold mb-1">{{ $t('swap.fromMetamaskAddress') }}
                  </div>
                  <div class="font-bold text-black text-tsm break-all mr-2">{{
                    isMetamaskConnected ? (currentAccount ? currentAccount : $t('swap.notConnected')) : $t('swap.notConnected') }}
                  </div>
                </div>
              </div>
            </div>
            <div class="md:grid hidden " :class="currentAccount ? 'grid-cols-2' : 'grid-cols-1'">
              <div
                class="border-l border-gray-200 text-green-500 font-semibold text-xxs w-16 lg:w-20 flex items-center justify-center uppercase"
                v-if="currentAccount">
                <div>
              ...
              <div
                class="border-l border-gray-200 text-blue-primary text-tsm font-bold w-16 lg:w-20 flex justify-center items-center">
                <div v-if="isInstallMetamask">
                  <div class="cursor-pointer" @click="connectMetamask()" v-if="!currentAccount">{{ $t('swap.connect') }}
                  </div>
            ...
            <div class="md:hidden">
              <div class="border-t border-gray-200 text-green-500 font-semibold text-xxs uppercase p-2"
                v-if="currentAccount">
                <div class="flex items-center justify-center">
                  <img src="@/modules/services/submodule/mainnetSwap/img/icon-connected.svg"
          ...
          <SupplyInputClean :disabled="disableAmount" v-model="amount" :balance="balance"
            :placeholder="'BEP20 ' + `${selectedToken ? selectedToken.name : ''}`" type="text" :showError="showAmountErr"
          ...
          <div v-if="toggleContact" class=" border ">
            <div class='text-xxs text-left text-gray-300 font-semibold py-2 px-2 uppercase'>{{ $t('general.importFromAB') }}
            </div>
            <div v-for="(item, number) in contacts" :key="number" class="cursor-pointer">
              <div @click="siriusAddress = item.value; toggleContact = false" class="flex justify-between">
                ...
                <div v-if="number % 2 == 0"
                  class="ml-auto pr-2 text-xxs py-2 font-semibold text-blue-primary bg-gray-100 uppercase">
                  {{ $t('general.select') }}</div>
                <div v-if="number % 2 == 1" class="ml-auto mr-2 text-xxs py-2 font-semibold text-blue-primary uppercase">
                  {{ $t('general.select') }}</div>
              </div>
            </div>
          </div>
          <div class="bg-blue-50 border border-blue-primary h-20 mt-5 rounded flex items-center justify-center">
            ...
            <img src="@/modules/account/img/metx-logo.svg"
              v-if="selectedToken ? selectedToken.name == 'metx' ? true : false : false" class=" w-5 h-5 ml-4">
            <img v-if="selectedToken ? selectedToken.name == 'xpx' ? true : false : false"
              src="@/modules/dashboard/img/icon-xpx.svg" class=" w-5 h-5 ml-4">
          </div>
          <div class="my-4 text-xs">Total Amount of {{ selectedToken ? selectedToken.name.toUpperCase() : '' }} received after
            deducting transaction fee</div>
          <div class="mt-10 text-center">
            <button @click="$router.push({ name: 'ViewServicesMainnetSwap' })"
              class="text-black font-bold text-xs mr-5 focus:outline-none disabled:opacity-50">{{ $t('general.cancel') }}</button>
            <button type="submit" class="default-btn focus:outline-none disabled:opacity-50" :disabled="isDisabledSwap"
              @click="sendRequest()">{{ $t('swap.sendRequest') }}</button>
          </div>
        </div>
        <div v-if="currentPage == 2">
          <div class="text-lg my-7">
            <div class="error error_box mb-5" v-if="err != ''">{{ err }}</div>
            <div class="font-bold text-left text-xs md:text-sm" :class="step1 ? 'text-gray-700' : 'text-gray-300'">
              {{ $t('swap.step1', { tokenName: selectedToken.name.toUpperCase() }) }}</div>
            <div class="flex border-b border-gray-300 p-3">
              <div class="flex-none">
                <div class=" rounded-full border w-6 h-6 transition-all duration-500"
                  :class="step1 ? 'border-blue-primary' : 'border-gray-300'">
                  <div class="flex h-full justify-center">
                    <font-awesome-icon icon="check" :class="step1 ? 'text-blue-primary' : 'text-gray-300'"
                      class="w-3 h-3 self-center inline-block transition-all duration-500"></font-awesome-icon>
                  </div>
                </div>
              </div>
              <div class="flex-grow text-left text-xs md:text-sm ml-3 self-center transition-all duration-500"
                :class="step1 ? 'text-gray-700' : 'text-gray-300'">{{ $t('swap.sendingTx') }}</div>
            </div>
            <div class="flex border-b border-gray-300 p-3">
              <div class="flex-none">
                <div class=" rounded-full border w-6 h-6 transition-all duration-500"
                  :class="isInvalidConfirmedMeta ? 'border-red-primary' : (step2 ? 'border-blue-primary' : 'border-gray-300')">
                  <div class="flex h-full justify-center">
                    <font-awesome-icon icon="times"
                      class="text-red-primary w-3 h-3 self-center inline-block transition-all duration-500"
                      v-if="isInvalidConfirmedMeta"></font-awesome-icon>
                    <font-awesome-icon icon="check" :class="step2 ? 'text-blue-primary' : 'text-gray-300'"
                      class="w-3 h-3 self-center inline-block transition-all duration-500" v-else></font-awesome-icon>
                  </div>
                </div>
              </div>
              <div class="flex-grow text-left text-xs md:text-sm ml-3 self-center transition-all duration-500"
                :class="step2 ? 'text-gray-700' : 'text-gray-300'">
                {{ isInvalidConfirmedMeta ? $t('swap.rejectMetamask') : $t('swap.waitingConfirmMetamask') }}
                <div v-if="isInvalidConfirmedMeta" class="mt-5">
                  <button type="button"
                    class="bg-blue-primary rounded mr-5 focus:outline-none text-tsm font-bold py-2 border-blue-primary px-8 text-white hover:shadow-lg"
                    @click="getValidation">{{ $t('swap.retry') }}</button>
                  <router-link :to="{ name: 'ViewServicesMainnetSwap' }"
                    class="px-6 py-2 text-gray-500 outline-none focus:outline-none mr-4 w-32 text-tsm"
                    tag="button">{{ $t('swap.cancelSwap') }}</router-link>
                </div>
              </div>
            </div>
            <div class="flex border-b border-gray-300 p-3">
              <div class="flex-none">
                <div class=" rounded-full border w-6 h-6" :class="step3 ? 'border-blue-primary' : 'border-gray-300'">
                  <div class="flex h-full justify-center">
                    <font-awesome-icon icon="check" :class="step3 ? 'text-blue-primary' : 'text-gray-300'"
                      class="w-3 h-3 self-center inline-block"></font-awesome-icon>
                  </div>
                </div>
              </div>
              <div class="text-left text-xs md:text-sm ml-3 self-center transition-all duration-500"
                :class="step3 ? 'text-gray-700' : 'text-gray-300'">
                {{ $t('swap.transactionHash') }}:
                <div v-if="validationHash" class="bg-yellow-100 py-2 px-5 mt-3 rounded flex">
                  <a :href="validationLink" target=_new
                    :class="isInvalidConfirmedMeta ? 'text-gray-300' : 'text-blue-primary'"
                    class="flex-grow break-all text-tsm self-center hover:underline" id="validateTransfer"
                    :copyValue="validationHash" :copySubject="$t('swap.transactionHash')"><font-awesome-icon
                      icon="external-link-alt"
                      class="text-blue-primary w-3 h-3 self-center inline-block mr-2"></font-awesome-icon>{{
                        validationHash }}</a>
                  <div class="flex-none mr-3">
                    <font-awesome-icon icon="copy" @click="copy('validateTransfer')"
                      class="w-5 h-5 text-blue-primary cursor-pointer self-center mx-3 absolute top-2 hover:opacity-90 duration-800 transition-all"
                      v-if="step3"></font-awesome-icon>
                  </div>
                </div>
                <div class="sm:flex">
                  <button
                    class="sm:flex-none justify-start sm:justify-end bg-blue-primary h-15 w-60 rounded mr-5 focus:outline-none text-xs font-bold py-2 border border-blue-primary px-8 text-white hover:shadow-lg mt-3 sm:mt-0 disabled:opacity-50 self-center"
                    type="button" v-if="validationHash" :disabled="isDisabledCheckTxnConfirmed || transactionFailed"
                    @click="triggerTxnConfirmation">{{ isCheckingTxnConfirmation ?
                      $t('swap.checkTransaction') : $t('swap.checkingTransaction') }}</button>
                  <div v-if="validationHash" class="py-2 sm:flex-grow text-xs mt-3">
                    ...
                    <div class="mb-1">{{ $t('swap.validationWarning6') }} <a :href="validationLink" target="_new"
                        class="text-blue-primary inline-block hover:text-blue-900 hover:underline">{{ $t('swap.bscScan') }}<font-awesome-icon
                          icon="external-link-alt"
                          class="ml-1 text-blue-primary w-3 h-3 self-center inline-block"></font-awesome-icon></a>.</div>
                  </div>
                </div>
                <div class="text-tsm mt-2 bg-blue-100 px-4 py-2 rounded-xl inline-block text-blue-900"
                  v-if="isTxnNotConfirmed && !transactionFailed">{{ $t('swap.notConfirmed') }}</div>
                <div class="text-tsm mt-2 bg-red-100 px-4 py-2 rounded-xl inline-block text-red-primary"
                  v-if="transactionFailed">{{ $t('swap.txFailed') }}</div>
              </div>
            </div>
            <div class="font-bold text-left text-xs md:text-sm mt-4" :class="step4 ? 'text-gray-700' : 'text-gray-300'">
              {{ $t('swap.step2') }}</div>
            <div class="flex border-b border-gray-300 p-3">
              <div class="flex-none">
                <div class=" rounded-full border w-6 h-6" :class="step4 ? 'border-blue-primary' : 'border-gray-300'">
                  <div class="flex h-full justify-center">
                    <font-awesome-icon icon="check" :class="step4 ? 'text-blue-primary' : 'text-gray-300'"
                      class="w-3 h-3 self-center inline-block"></font-awesome-icon>
                  </div>
                </div>
              </div>
              <div class="flex-grow text-left text-xs md:text-sm ml-3 self-center transition-all duration-500"
                :class="step4 ? 'text-gray-700' : 'text-gray-300'">{{ $t('swap.sendingMsg') }}</div>
            </div>
            <div class="flex border-b border-gray-300 p-3">
              <div class="flex-none">
                <div class=" rounded-full border w-6 h-6"
                  :class="isInvalidSignedMeta ? 'border-red-primary' : (step5 ? 'border-blue-primary' : 'border-gray-300')">
                  <div class="flex h-full justify-center">
                    <font-awesome-icon icon="times" class="text-red-primary w-3 h-3 self-center inline-block"
                      v-if="isInvalidSignedMeta"></font-awesome-icon>
                    <font-awesome-icon icon="check" :class="step5 ? 'text-blue-primary' : 'text-gray-300'"
                      class="w-3 h-3 self-center inline-block" v-else></font-awesome-icon>
                  </div>
                </div>
              </div>
              <div class="flex-grow text-left text-xs md:text-sm ml-3 self-center transition-all duration-500"
                :class="step5 ? 'text-gray-700' : 'text-gray-300'">
                {{ signatureMessage }}
                <div v-if="isInvalidSignedMeta" class="mt-5">
                  <button type="button"
                    class="bg-blue-primary rounded mr-5 focus:outline-none text-tsm py-2 px-4 text-white hover:shadow-lg w-24"
                    @click="getSigned">{{ $t('swap.retry') }}</button>
                </div>
              </div>
            </div>
            <div class="flex border-b border-gray-300 p-3">
              <div class="flex-none">
                <div class=" rounded-full border w-6 h-6" :class="step6 ? 'border-blue-primary' : 'border-gray-300'">
                  <div class="flex h-full justify-center">
                    <font-awesome-icon icon="check" :class="step6 ? 'text-blue-primary' : 'text-gray-300'"
                      class="w-3 h-3 self-center inline-block"></font-awesome-icon>
                  </div>
                </div>
              </div>
              <div class="flex-grow text-left text-xs md:text-sm ml-3 self-center transition-all duration-500"
                :class="step6 ? 'text-gray-700' : 'text-gray-300'">{{ $t('swap.messageSigned') }}<div
                  class="bg-yellow-100 py-2 px-5 mt-2 rounded flex" v-if="messageHash && step6">
                  <div :class="step6 ? 'text-gray-500' : 'text-gray-300'" class="text-tsm break-all flex-grow"
                    id="validateMessage" :copyValue="messageHash" :copySubject="$t('swap.signatureHash')">{{ messageHash
                    }}</div>
                  <div class="flex-none mr-3"><font-awesome-icon icon="copy" @click="copy('validateMessage')"
                      class="w-5 h-5 text-blue-primary cursor-pointer self-center ml-3 absolute top-2 hover:opacity-90 duration-800 transition-all"
                      v-if="step6"></font-awesome-icon></div>
                </div>
              </div>
            </div>
            <div class="font-bold text-left text-xs md:text-sm mt-4" :class="step7 ? 'text-gray-700' : 'text-gray-300'">
              {{ $t('swap.step3') }}</div>
            <div class="flex border-b border-gray-300 p-3">
              <div class="flex-none">
                <div class=" rounded-full border w-6 h-6"
                  :class="isInvalidSwapService ? 'border-red-primary' : (step7 ? 'border-blue-primary' : 'border-gray-300')">
                  <div class="flex h-full justify-center">
                    <font-awesome-icon icon="times" class="text-red-primary w-3 h-3 self-center inline-block"
                      v-if="isInvalidSwapService"></font-awesome-icon>
                    <font-awesome-icon icon="check" :class="step7 ? 'text-blue-primary' : 'text-gray-300'"
                      class="w-3 h-3 self-center inline-block" v-else></font-awesome-icon>
                  </div>
                </div>
              </div>
              <div class="flex-grow text-left text-xs md:text-sm ml-3 self-center transition-all duration-500"
                :class="step7 ? 'text-gray-700' : 'text-gray-300'">
                {{ isInvalidSwapService ? $t('swap.sendMsgFail') : $t('swap.messageSent') }}
                <div v-if="isInvalidSwapService && swapServerErrIndex <= 3" class="mt-5">
                  <button type="button"
                    class="bg-blue-primary rounded mr-5 focus:outline-none text-tmd py-2 px-4 text-white hover:shadow-lg disabled:opacity-50"
                    @click="afterSigned" :disabled="disableRetrySwap">{{ retrySwapButtonText }}</button>
                </div>
                <div v-if="swapServerErrIndex > 3" class="mt-5 text-tsm sm:text-sm">
                  {{ $t('swap.serverErrMsg1') }} <b>{{ $t('swap.transactionHash') }}</b>, <b>{{ $t('swap.signature') }}</b>
                  {{ $t('swap.serverErrMsg2') }} <a href="https://t.me/proximaxhelpdesk" target=_new
                    class="text-blue-primary font-bold underline">{{ $t('home.helpdesk') }}</a>.
                </div>
              </div>
            </div>
          </div>
          <div class="mt-10 text-center">
            <button type="submit" class="default-btn focus:outline-none disabled:opacity-50"
              :disabled="isDisabledValidate" @click="validated()">{{ $t('general.continue') }}</button>
          </div>
        </div>
        <div v-if="currentPage == 3">
          <div>
            ...
            <swap-certificate-component :networkTerm="$t('swap.bsc')" swapType="In" :swapToken="swapToken"
              :swapTimestamp="swapTimestamp" :transactionHash="transactionHash" :siriusName="siriusName"
              :swappedAmount="Number(amount)" :siriusAddress="Helper.createAddress(siriusAddress).pretty()"
              :swapQr="swapQr" :swapLink="validationLink" />
            <button type="button"
              class="w-40 hover:shadow-lg bg-blue-primary text-white text-xs hover:opacity-50 rounded font-bold px-4 py-3 border border-blue-primary outline-none mr-4 mt-6"
              @click="saveCertificate">{{ $t('general.certificate') }}</button>
            <div class="mt-3">
              <a :href="validationLink" target=_new
                class="underline self-center text-xs font-bold text-blue-primary">{{ $t('swap.viewTxInExplorer') }}<font-awesome-icon
                  icon="external-link-alt"
                  class="ml-2 text-blue-500 w-3 h-3 self-center inline-block"></font-awesome-icon></a>
            </div>
            <div
              class="md:mx-20 lg:mx-40 font-bold text-center text-tsm py-5 sm:py-10 mt-5 sm:mt-10 border-t border-gray-200">
              {{ $t('swap.swapDetails') }}</div>
            <div class="md:mx-20 lg:mx-10 xl:mx-40 border-2 border-gray-200 mt-4 p-5 text-xs font-bold filter shadow-lg">
              ...
              <div class="mt-1">{{ $t('swap.equivalentTo') }} {{ amountReceived }} {{ selectedToken.name.toUpperCase() }}
                <img src="@/modules/account/img/metx-logo.svg" v-if="selectedToken.name == 'metx'"
                  class="w-3 h-3 ml-2 inline relative" style="top: -2px">
                <img v-else-if="selectedToken.name == 'xpx'" src="@/modules/dashboard/img/icon-xpx.svg"
                  class="w-3 h-3 ml-2 inline relative" style="top: -2px">
              </div>
            </div>
            ...
            <div class="sm:mt-3 text-center">
              <router-link :to="{ name: 'ViewServicesMainnetSwap' }">
                <button type="submit" class="default-btn mr-5 focus:outline-none w-40 inline-block disabled:opacity-50"
                  :disabled="!savedCheck">{{ $t('general.done') }}</button>
              </router-link>
            </div>
          </div>
        </div>
...
const accounts = computed<{ name: string; balance: number; address: string; publicKey: string; isMultisig: boolean; }[]>(() => {
  if (!walletState.currentLoggedInWallet) {
    return [];
  }
...
const contacts = computed(() => {
  ...
  var contact: Array<{ value: string, label: string }> = [];
...
const selectedToken = ref<{ name: string }>({ name: "" });
const tokenList = ref<{ name: string, assetId: string, contractAddress: string, namespace: string }[]>([])
const getBscSwapTokenList = async () => {

  let tokens = await SwapUtils.getSwapTokenList(swapData.swap_XPX_BSC_URL)
  if (tokens) {

    tokenList.value = tokens.map((token: { name: string, assetId: string, contractAddress: string, namespace: string }) => {
      return {
        name: token.name,
...
function verifyChain(chainId: string, updateTokenBol = false) {
...
const showAmountErr = computed(() => {
  if (Number(amount.value) <= balance.value && Number(amount.value) >= minAmount.value) {
    return false
  } else {
    return true
  }
...
const isDisabledSwap = computed(() =>
  ...
  !(Number(amount.value) > 0 && showAddressError.value == false && !err.value && (balance.value >= Number(amount.value)) && Number(amount.value) >= minAmount.value)
);
...
const amountReceived = computed(() => {
  if (Math.round((Number(amount.value) - feeAmount.value) * Math.pow(10, tokenDivisibility.value)) / Math.pow(10, tokenDivisibility.value) > 0) {
    return Math.round((Number(amount.value) - feeAmount.value) * Math.pow(10, tokenDivisibility.value)) / Math.pow(10, tokenDivisibility.value)
  } else {
    return 0;
  }
});
...
const swapServiceParam = ref<{ recipient: string; signature: any; txnInfo: { network: string; txnHash: string; }; }>();
...
const afterSigned = async () => {
  ...
  try {
    ...
    if (response.status == 200 || response.status == 201) {
      ...
      SwapUtils.generateQRCode(validationLink.value).then((x) => {
        swapQr.value = x
      });;
