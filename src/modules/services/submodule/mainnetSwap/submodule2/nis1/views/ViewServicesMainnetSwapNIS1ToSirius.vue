  ...
  <div>
    <div class='lg:w-9/12 ml-2 mr-2 lg:ml-auto lg:mr-auto mt-5'>
      <div class='mt-6 p-6 border filter shadow-lg text-center'>
        ...
        <div class="text-xs my-3 mb-5 sm:mb-10"><img src="@/modules/services/submodule/mainnetSwap/submodule2/nis1/img/nem.png" class="mr-2 h-5 inline-block">{{$t('nis1.swapNis1ToSirius')}}</div>
        ...
        <div v-if="currentPage==1">
          ...
          <form @submit.prevent="swap">
            <SelectInputNIS1Account v-model="siriusAddressToSwap" :placeholder="$t('nis1.fromNis1Acc')" />
            <div class="relative">
              ...
              <SwapInputClean class="mt-5" :disabled="disableAmount" v-model="amount" :balance="selectedAccountNis1Balance" :placeholder="currentSwapTokenName + ' ' + $t('general.amount')" type="text" :showError="showAmountErr" :errorMessage="$t('general.insufficientBalance')" emptyErrorMessage="Amount is empty" :maxAmount="maxSwapAmount" :decimal="swapDecimal" @clickedMaxAvailable="updateAmountToMax()" :remarkOption="false" :toolTip="$t('nis1.amountMsg')" />
              <!-- <div class="text-left">
              ...
              <div class="mt-5">
                <SelectInputAccountOutgoingSwap v-model="siriusAddress" :placeholder="$t('swap.toSiriusAcc')" :selectDefault="walletState.currentLoggedInWallet.selectDefaultAccount().address" />
              </div>
              <PasswordInputClean :placeholder="$t('general.password')" :errorMessage="$t('general.passwordRequired')" :showError="showPasswdError" v-model="walletPasswd" class="mt-5" />
              ...
              <div class="flex justify-center mt-3">
                <div class="text-xs text-gray-600 mt-2 max-w-screen-md">{{$t('swap.swapMsg')}}</div>
              </div>
              <div class="mt-10">
                <button @click="$router.push({name: 'ViewServices'})" class="default-btn mr-5 focus:outline-none disabled:opacity-50" :disabled="isDisabledCancel">{{$t('general.later')}}</button>
        ...
        <div v-if="currentPage==2">
          <div>
            ...
            <swap-certificate-component :networkTerm="$t('nis1.nis1')" swapType="In" swapId="" swapTimestamp="" :transactionHash="nis1Hash" :siriusName="siriusName" :swappedAmount="amount" :siriusAddress="Helper.createAddress(siriusAddress).pretty()" :swapQr="swapQr" :swapLink="validationLink" />
            ...
            <div class="md:mx-20 lg:mx-10 xl:mx-40 border-2 border-gray-200 mt-4 p-5 text-xs font-bold filter shadow-lg">
              <div class="text-blue-primary mb-1">{{$t('general.from')}}: {{$t('nis1.nis1Acc')}}</div>
              <div class="break-all">{{ Helper.createAddress(siriusAddressToSwap).pretty() }}</div>
              <div class="mt-1">{{$t('swap.swapAmount')}} {{ amount }} {{ currentSwapTokenName.toUpperCase() }}</div>
              <div>
              ...
              <div class="mt-1">{{$t('swap.equivalentTo')}} {{ amount }} {{ currentNativeTokenName }} <img src="@/modules/dashboard/img/icon-xpx.svg" class="w-3 h-3 ml-2 inline relative" style="top: -2px"></div>
            </div>
