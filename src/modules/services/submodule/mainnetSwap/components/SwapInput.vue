  ...
  <div>
    <div class="text-outline bg-white" :class="borderColor">
      <div class="text-icon-outline text-icon">
        ...
        <div class="ml-6 text-xs text-gray-500 text-left" style="width: 170px"><button class="border border-gray-200 bg-gray-50 py-1 px-3 rounded-md cursor-pointer relative hover:bg-gray-100 hover:shadow-sm focus:outline-none" style="top: -3px" @click="showRemark();$emit('clickedMaxAvailable', true);">{{$t('swap.maxamount')}}</button></div>
      </div>
    ...
    <div class="float-right text-gray-600 text-xs mr-3 my-2" v-if="isShowRemark==true && remarkOption">{{$t('swap.gasmessage')}} <b>[[transaction fee]] xpx</b> {{$t('swap.gasmessage2')}} <b>[[gas fee]]xpx</b></div>
  </div>
</template>
