  ...
  <div class='w-10/12 ml-auto mr-auto'>
    <div class="border filter shadow-lg xl:grid xl:grid-cols-3 mt-8" >
      <div class="xl:col-span-2 p-12">
          ...
          <div class="flex gap-1 mt-3">
            ...
            <div v-if="selectedMultisigAddress" class="mt-3">
              <MultisigInput :select-default-address="selectedMultisigAddress"
                :select-default-name="selectedMultisigName" label="Multisig account selected" :type="'dynamic'"/>
            </div>   
        </div>
        <div class="mt-3">
        ...
        <div class="">
          <PublicKeyInputClean placeholder="Harvester Key" v-model="harvesterKey" v-debounce:1000="checkHarvester" :showError="showHarvesterError" />
        </div>
      </div>
...
import { AddHarvesterTransactionBuilder, Address, HarvesterInfo, 
  PublicAccount, RemoveHarvesterTransactionBuilder, UInt64 } from 'tsjs-xpx-chain-sdk';
...
const checkHarvester = ()=>{
  try {
    harvesterKey.value = PublicAccount.createFromPublicKey(harvesterKey.value, AppState.networkType).publicKey;
...
/** */
  ...
  const transactionFee = computed(()=>{
    if(!AppState.isReady){
      ...
      if(selectedMultisigAddress.value){
        let aggTxn = AppState.buildTxn.aggregateBonded([txn.toAggregateV1(Helper.samplePubAcc())]);
        return Helper.amountFormatterSimple(aggTxn.maxFee.compact(), AppState.nativeToken.divisibility);
      }
      else{
        return Helper.amountFormatterSimple(txn.maxFee.compact(), AppState.nativeToken.divisibility);
      }
    }
  })

  const transactionFeeExact = computed(()=>{
    return Helper.convertNumberMinimumFormat(Number(transactionFee.value), AppState.nativeToken.divisibility);
  })
  ...
  const createTxn = async () => {
    ...
    try {
      const harvesterInfo: HarvesterInfo[] = await 
        AppState.chainAPI.harvesterAPI.getAccountHarvestingHarvesterInfo(
        ...
        if(isAdding && harvesterInfo.length){
          ...
          toast.add({severity:'error', detail: "Harvester key found, it is already added", group: 'br-custom', life: 10000});
          return;
        }
        else if(!isAdding && harvesterInfo.length === 0){
          // harvester not exist, no need remove
          toast.add({severity:'error', detail: "Harvester key not found, does not need to be removed", group: 'br-custom', life: 10000});
          return;
        }
    } catch (error) {
      return;
    ...
    if(selectedMultisigAddress.value){
      ...
      txnObj = TransactionUtils.signTxnWithPassword(selectedAddress.value,selectedMultisigAddress.value,walletPassword.value,null,innerTxns, new UInt64(nodeTime.sendTimeStamp!));
    }else{
