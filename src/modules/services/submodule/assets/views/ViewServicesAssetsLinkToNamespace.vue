  ...
  <TransactionLayout :type="'account'">
    <template #white>
      ...
      <div class="text-right w-full">
        <div
          v-if="getMultiSigCosigner.cosignerList.length > 0"
          class="inline-block"
        >
      ...
      <div class="border border-gray-200 p-4 rounded mt-5">
        <div class="lg:grid lg:grid-cols-2">
          <div class="my-3">
            <div class="text-xxs text-blue-primary uppercase mb-1 font-bold">
              {{ $t("asset.currentSupply")
              }}<img
                src="@/assets/img/icon-info.svg"
                class="inline-block ml-2 relative"
                style="top: -1px"
                v-tooltip.bottom="{
                  value:
                    '<tiptext>' +
            ...
            <div class="text-xxs text-blue-primary uppercase mb-1 font-bold">
              {{ $t("general.divisibility")
                ...
                v-tooltip.bottom="{
                  value:
                    '<tiptext>' +
                    $t('asset.divisibilityMsg4') +
                    '<br><br>' +
                    $t('asset.divisibilityMsg2') +
                    '<br>' +
                    $t('asset.divisibilityMsg3') +
                    '</tiptext>',
                  escape: false,
                }"
              />
          ...
          <div class="my-3">
            <div class="text-xxs text-blue-primary uppercase mb-1 font-bold">
              {{ $t("general.transferable")
                ...
                v-tooltip.bottom="{
                  value:
                    '<tiptext>' + $t('asset.transferableMsg') + '</tiptext>',
                  escape: false,
                }"
              />
            </div>
            <div class="uppercase text-black font-bold text-sm">
              {{ assetTransferable ? $t("general.yes") : $t("general.no") }}
            </div>
          </div>
          <div class="my-3">
            <div class="text-xxs text-blue-primary uppercase mb-1 font-bold">
              {{ $t("general.supplyMutable")
              }}<img
                src="@/assets/img/icon-info.svg"
                class="inline-block ml-2 relative"
                style="top: -1px"
                v-tooltip.bottom="{
                  value:
                    '<tiptext>' + $t('asset.supplyMutableMsg') + '</tiptext>',
                  escape: false,
                }"
              />
            </div>
            <div class="uppercase text-black font-bold text-sm">
              {{ assetMutable ? $t("general.yes") : $t("general.no") }}
            </div>
          </div>
        </div>
      </div>
      <div class="lg:grid lg:grid-cols-2 mt-5">
        <SelectLinkType
          :title="$t('asset.modificationType')"
          class="lg:mr-4"
...
export default {
  ...
  setup(props) {
    ...
    const disableCreate = computed(() => !(selectNamespace.value != ""));
    ...
    if (account.value) {
      let asset = account.value.assets.find(
        (asset) => asset.idHex === props.assetId
      );
      if (asset != undefined) {
        ...
        assetSupply.value = Helper.convertToCurrency(
          asset.supply,
          asset.divisibility
        );
    ...
    try {
      transactionFee.value = Helper.convertToCurrency(
        AssetsUtils.getMosaicSupplyChangeTransactionFee(
          selectAsset.value,
          selectAction.value,
          supply.value,
          assetDivisibility.value
        ),
        AppState.nativeToken.divisibility
      );
    ...
    const linkNamespace = () => {
      ...
      if (selectAction.value == "link") {
        ...
        let account = walletState.currentLoggedInWallet.accounts.find(
          (account) => account.address == selectedAccAdd.value
        );
        if (!account) {
          account = walletState.currentLoggedInWallet.others.find(
            (account) => account.address == selectedAccAdd.value
          );
        }
        assetId = account.namespaces.find(
          (namespace) => namespace.name === selectNamespace.value
        ).linkedId;
      }
      const buildTransactions = AppState.buildTxn;
      let aliasActionType =
        selectAction.value == "link"
          ? AliasActionType.Link
          : AliasActionType.Unlink;
      const unsignedLinkAssetToNamespaceTx = buildTransactions.assetAlias(
        aliasActionType,
        new NamespaceId(selectNamespace.value),
      ...
      TransactionState.unsignedTransactionPayload =
        unsignedLinkAssetToNamespaceTx.serialize();
