      ...
      <form @submit.prevent="modifyMosaic">
        <fieldset class="w-full">
          ...
          <div class="mt-10">
            ...
            <button type="button" class="default-btn py-1 disabled:opacity-50" :disabled="disableCreate" @click="modifyMosaic">Confirm</button>
          </div>
        </fieldset>
      </form>
    </div>
...
export default {
  ...
  setup(){
    ...
    const increaseDecreaseOption = () => {
      let action = [];
      action.push({value: 'decrease', label: 'Decrease'});
      action.push({value: 'increase', label: 'Increase'});
      return action;
    };
