  ...
  <TransactionLayout>
    <template #white>
      ...
      <div class="mt-4">
        ...
        <div class="lg:grid lg:grid-cols-2 mt-5">
          <div class="lg:mr-2">
            <SupplyInputClean
              :disabled="showNoBalance || disabledInput"
              v-model="supply"
              :balance="Number.MAX_VALUE"
              :placeholder="$t('general.supply')"
              type="text"
              @show-error="updateSupplyErr"
              :decimal="Number(divisibility)"
              :toolTip="
                $t('asset.supplyMsg1') +
                ' <br><br>' +
                $t('asset.supplyMsg2') +
          ...
          <div class="lg:ml-2">
            <NumberInputClean
              ...
              :max="6"
              :placeholder="$t('general.divisibility')"
              :showError="showDivisibilityErr"
              :toolTip="
                $t('asset.divisibilityMsg1') +
                ' <br><br>' +
        ...
        <div class="lg:grid lg:grid-cols-2">
          <div class="mb-5 lg:mb-0 lg:mr-2">
            <CheckInput
              ...
              :title="$t('general.transferable')"
              :toolTip="$t('asset.transferableMsg')"
          ...
          <div class="mb-5 lg:mb-0 lg:ml-2">
            <CheckInput
              :disabled="showNoBalance || disabledInput"
              v-model="isMutable"
              :title="$t('general.supplyMutable')"
              :toolTip="$t('asset.supplyMutableMsg')"
    ...
    <template #navy>
      <TxnSummary
        ...
        :asset-rental-fee-currency="rentalFeeCurrency"
    ...
    <template #description>
      <div class="mb-8">
        <a
          ...
          target="_new"
          class="sm:h-9 lg:h-5 text-blue-primary font-bold text-tsm flex items-start"
          >{{ $t("general.assetQues") }}</a
        >
        <div class="text-gray-400 text-xs lg:text-tsm my-3 sm:pr-2">
          {{ $t("asset.assetAns") }}
        </div>
      </div>
      <div class="mb-8">
        <a
          ...
          target="_new"
          class="sm:h-9 lg:h-5 text-blue-primary font-bold text-tsm items-start flex"
          >{{ $t("general.feedback") }}</a
        >
        <div class="text-gray-400 text-tsm my-3">
          {{ $t("general.feedbackDescription") }}
        </div>
      </div>
    </template>
  </TransactionLayout>
...
try {
  transactionFee.value = Helper.amountFormatterSimple(
    AssetsUtils.createAssetTransactionFee(
...
const rentalFee = computed(() => {
  if (networkState.currentNetworkProfileConfig) {
    return Helper.convertToExact(
      networkState.currentNetworkProfileConfig.mosaicRentalFee,
      AppState.nativeToken.divisibility
    );
  } else {
    return 0;
...
const disableCreate = computed(
  () =>
    !(
      ...
      divisibility.value != null &&
      Number(supply.value) > 0 &&
      !showSupplyErr.value &&
...
const createAsset = () => {
  if (selectedMultisigAddress.value) {
    const assetDefinition = AppState.buildTxn.mosaicDefinition(
      multisigPublicAccount.value,
      isMutable.value,
      isTransferable.value,
      Number(divisibility.value)
    );
    let supplyChangeType: MosaicSupplyType = MosaicSupplyType.Increase;
    const assetSupplyChangeTx = AppState.buildTxn.buildMosaicSupplyChange(
      assetDefinition.mosaicId,
      supplyChangeType,
      UInt64.fromUint(
        AssetsUtils.addZeros(Number(divisibility.value), Number(supply.value))
      )
    );
    const unsignedInnerTxn = [
      assetDefinition.serialize(),
      assetSupplyChangeTx.serialize(),
    ];
    ...
    const assetDefinitionTx = assetDefinition.toAggregateV1(
      ownerPublicAccount.value
    );
    let supplyChangeType: MosaicSupplyType = MosaicSupplyType.Increase;
    const assetSupplyChangeTx = AppState.buildTxn
      ...
      .toAggregateV1(ownerPublicAccount.value);
    let unsignedAssetAggregateTransaction = AppState.buildTxn
      .aggregateComplete([assetDefinitionTx, assetSupplyChangeTx])
      .serialize();
    TransactionState.unsignedTransactionPayload =
      unsignedAssetAggregateTransaction;
  }
  clearInput();
