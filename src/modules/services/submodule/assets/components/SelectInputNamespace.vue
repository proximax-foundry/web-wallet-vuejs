<template>
  ...
  <div v-if='toggleSelection' class='absolute border border-t-0 w-full z-50 bg-white max-h-40 overflow-auto px-3 filter drop-shodow-xl'>
    ...
    <div v-for='(items,index) in namespaces' :key="items" class="px-2 py-3 flex cursor-pointer items-center" @click="selectNamespace(items.label, items.value);$emit('update:modelValue', selectedNamespace);$emit('select-account', selectedNamespace);" :class='`${(index != accounts.length - 1)?"border-b border-gray-200":""}`'>
      <div v-html="toSvg(items.value, 20, jdenticonconfig)"></div>
      <div class='text-xs ml-2 font-semibold'>{{items.label}}</div>
      <div v-if='items.label!=selectedNamespace' class='cursor-pointer text-blue-primary text-xxs mt-0.5 ml-auto font-semibold'>SELECT</div>
...
export default defineComponent({
  ...
  setup(props){
    ...
    const selectNamespace = (accountName, accountAddress) => {
      selectNamespace.value = accountName;
      selectedNamespace.value = accountAddress;
      toggleSelection.value = !toggleSelection.value;
    };

    const namespaces = computed(() => {
      console.log(AssetsUtils.listActiveNamespacesToLink(props.address, props.action));
      return AssetsUtils.listActiveNamespacesToLink(props.address, props.action);
    });
