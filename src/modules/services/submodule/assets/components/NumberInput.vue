  ...
  <div>
    <div class="text-outline bg-white" :class="borderColor">
      <div class="text-icon-outline text-icon">
        ...
        <div class="ml-6 text-xs mt-1 text-gray-500 w-24">{{ title }}</div>
      </div>
      ...
      <!-- @keydown="validate" -->
      <input :disabled="disabled" :value="modelValue" @input="validate" @keypress="validateKey" type="number" size="1" maxlength="1" min=0 :max="max" :placeholder="placeholder" class="text-placeholder bg-white text-right" @click="clickInputText()" @focus="$event.target.select()">
      <div class="w-5"></div>
    </div>
