    ...
    <div class="bg-white border rounded-lg border-gray-200 shadow-lg group ">
      ...
      <div class="flex justify-between items-center">
        <div class="ml-4 mt-5 mb-5 mr-5 text-txs text-left">
          ...
          <div class="text-txs pt-1 text-black-400">{{ $t('wallet.numberOfAccounts') }}: <span>{{ wallet.accounts.length
          }}</span></div>
        </div>
        <div v-if="isLogin" class="xl:mr-3 xl:ml-0 text-xs font-bold">
          <button class="bg-transparent font-bold py-2 ml-2 mr-2 rounded inline-flex items-center"
            @click="exportWallet(wallet.name)">
            <svg class="color-blue-primary" id="file_upload_black_24dp" xmlns="http://www.w3.org/2000/svg" width="15"
              height="15" viewBox="0 0 20 20">
              <path
                d="M15.667,13.167v2.5h-10v-2.5H4v2.5a1.672,1.672,0,0,0,1.667,1.667h10a1.672,1.672,0,0,0,1.667-1.667v-2.5Zm-9.167-5L7.675,9.342l2.158-2.15V14H11.5V7.192l2.158,2.15,1.175-1.175L10.667,4Z"
                fill="#007cff" />
            </svg>
...
const exportWallet = (walletName :string) => {
