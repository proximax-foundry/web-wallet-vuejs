<template>
  <div class="container mx-auto text-center">
    <div class="mx-auto lg:mt-16">
      ...
      <p class="mt-4 text-sm">{{$t('deletewallet.accountsavailable')}}:</p>
      <div class="px-2 sm:px-0 text-left inline-block mt-3 sm:mt-5" v-if="accountsList.length > 0">
        <div v-for="i in accountsList" :key="i.address" class="bg-white rounded-md p-2 sm:p-5 w-full sm:w-82">
          <div class="font-bold text-xs mb-2">{{ i.name }}</div>
          <div class="mb-2 text-xs break-all">{{ i.address }}</div>
        </div>
      </div>
      <div class="my-5 mx-1 text-sm">{{$t('deletewallet.question')}}</div>
