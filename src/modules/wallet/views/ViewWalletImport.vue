  ...
  <div>
    <div class="container mx-auto md:grid md:grid-cols-2 md:mt-10 lg:px-20 xl:px-40 gap-4">
      ...
      <div class="md:col-span-1 bg-white mx-5 md:mx-0 px-30 pt-1 md:pt-0 rounded-md">
        <router-link :to="{ name: 'ViewWalletCreateSelection' }" class="text-xs m-2 text-blue-link items-center flex"><img
            src="@/assets/img/chevron_left.svg" class="w-5 inline-block">Back</router-link>
        <div class="text-lg text-center mt-16 font-semibold">{{ $t('wallet.createWallet') }}</div>
        <div class="text-xxs text-center text-blue-primary font-bold uppercase">{{ $t('wallet.fromBackUp') }}</div>
        <div class="text-xs mt-8 ml-auto mr-auto w-8/12 text-center mb-5">{{ $t('wallet.backUpDescription') }}</div>
        <SelectNetworkInput />
        <label class="cursor-pointer">
          <span
            class='mt-3 font-bold text-xs text-center blue-btn py-2 px-10  block ml-auto mr-auto w-8/12'>{{ $t('wallet.importFile') }}</span>
          <input type="file" @change="readWalletBackup($event)" ref="walletFile" hidden />
        </label>
        <div class='text-center text-xs mt-6 mb-1 '>{{ $t('wallet.haveWallet') }}</div>
        <div class="text-center  text-xs text-blue-primary font-semibold"><router-link
            :to="{ name: 'Home' }">{{ $t('wallet.signInHere') }} ></router-link></div>
        <div class='h-28'></div>
      </div>
    </div>
  </div>
...
const readWalletBackup = (e :Event ) => {
  ...
  reader.onload = e => {
    ...
    const file = CryptoJS.enc.Base64.parse(result);
    try {
      ...
      if (dataDecryp.networkName === undefined || dataDecryp.networkName !== selectedNetworkName.value) {
        confirm.require({
          message: t('wallet.importMsg', { network: selectedNetworkName.value }),
...
const importBackup = (dataDecryp :any) => {
