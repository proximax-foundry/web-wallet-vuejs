...
<div>
 <div v-if="!newWallet"  class="container mx-auto md:grid md:grid-cols-2 md:mt-10 lg:px-20 xl:px-40 gap-4">
    ...
    <div class="md:col-span-1 bg-white mx-5 md:mx-0 px-30 pt-1 md:pt-0 rounded-md">
      <router-link :to="{ name: 'ViewWalletCreateSelection' }" class="text-xs m-2 text-blue-primary items-center flex"><img src="@/assets/img/chevron_left.svg" class="w-5 inline-block">{{$t('general.back')}}</router-link>
       <form
      @submit.prevent="createWallet"
    >
      <div class="text-sm text-center mt-20 font-semibold">{{$t('wallet.createWallet')}} </div>
      <div class="text-xxs text-center text-blue-primary mb-5 font-bold uppercase">{{$t('general.fromPrivateKey')}}</div>
      ...
      <div class="mt-3 w-8/12 ml-auto mr-auto">
          <PasswordInput  :placeholder="$t('general.privateKey')" :errorMessage="$t('general.invalidPrivateKey')" icon="key" :showError="showPkError" v-model="privateKeyInput"  />
          <TextInput  class="mt-3" :placeholder="$t('wallet.namePlaceholder')" :errorMessage="$t('wallet.nameErrMsg')"  v-model="walletName" icon="wallet" />
      ...
      <div class='border-2 shadow-lg filter mb-10 bg-white'>
        <div class='flex'>
          ...
          <div class='flex flex-col justify-center ml-4'>
            ...
            <div class= 'flex'>
              <div id="address" :copyValue="address" :copySubject="$t('general.address')" class = 'text-xs font-semibold mt-1'>{{address}} </div>
