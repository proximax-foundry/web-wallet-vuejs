    ...
    <div class='md:w-8/12 lg:w-10/12 xl:w-6/12 ml-2 mr-2 md:ml-auto md:mr-auto mt-5'>
      <div class='border-2 border-gray-200'>
        ...
        <div class='w-full text-center pt-10 pl-10 pr-10' v-else-if="!invalidCosigner">
          ...
          <div class="mt-5 text-tsm">{{ $t('transaction.approveConfirmation', innerTransactions.length) }} <div
              class="inline-block h-3 w-3 bg-yellow-300 ml-1"></div>)</div>
          <div class="mt-1 text-tsm font-bold">{{ $t('general.deadline') }}: {{ deadline }}</div>
        </div>
        ...
        <div class='w-full text-center pb-10 pl-10 pr-10'>
          <div class="mt-10">
            <div class='flex items-center border-t border-gray-200 py-2' v-for="cosigner, item in allTxnCosigners"
              :key="item">
              <img src="@/modules/transaction/img/digital-signature-success.png" class="w-14 inline-block ml-2"
                v-if="isHasSigned(cosigner)">
              ...
              <div class="text-left ml-3 inline-block">
                <div class="uppercase text-blue-primary text-txs font-bold">{{ displayAccountLabel(cosigner) }} {{
                  isHasSigned(cosigner) ? '(' + $t('general.signed') + ')' : '' }}</div>
      ...
      <div class='border-2 mt-5'>
        <div class="cursor-pointer flex justify-between p-3" @click="viewTxn = !viewTxn">
          ...
          <div class="text-xs text-blue-primary uppercase flex justify-evenly items-center">{{ $t('general.view') }}<img
              src="@/modules/transaction/img/icon-down-caret.svg" class="ml-2 transition-all duration-200"
              :class="`${viewTxn ? 'rotate-180 transform' : ''}`"></div>
        </div>
        <transition name="slide">
          <div class="p-3" :class="`${innerTransactions.length > 0 ? 'border-t-2 border-gray-200' : ''}`" v-if="viewTxn">
            ...
            <div class="table_div pb-5"
              :class="`${innerTransactions.length > 0 ? 'border-b-2 border-gray-200 mb-5' : ''}`">
              <div>
            ...
            <div v-if="innerTransactions.length > 0">
              <div class="mt-10">{{ $t('general.transaction', innerTransactions.length) }} ({{ innerTransactions.length
              }})
              </div>
              <div class="mt-3 border border-gray-200 p-1" v-for="(item, index) in innerTransactions" :key="index">
                <div class="table_div" :class="innerRelatedList[index] ? 'highlighted' : ''">
                  ...
                  <div v-if="item.infoGreenList && item.infoGreenList.length > 0">
                    ...
                    <div v-else-if="item.legendType === InnerTxnLegendType.ALLOW_BLOCK">{{ $t('transaction.allow') }}
                    </div>
                    <div>{{ item.infoGreenList.map(info => info.short ? info.short : info.value).join(", ") }}</div>
                  </div>
                  <div v-if="item.infoRedList && item.infoRedList.length > 0">
                  ...
                  <div v-if="item.infoInfoList && item.infoInfoList.length > 0">
                    ...
                    <div>
                      <div v-for="(element, transfer) in item.sdas" :key="transfer">
                        <div class="flex items-center">
                          ...
                          <img v-if="element.toLowerCase().includes('xpx')" src="@/modules/account/img/proximax-logo.svg"
                            class="inline-block h-7 w-7 mx-2 border-2 rounded-3xl">
                          <img v-else-if="element.toLowerCase().includes('xar')"
                            src="@/modules/account/img/xarcade-logo.svg"
                            class="inline-block h-7 w-7 mx-2 border-2 rounded-3xl">
                          <img v-else-if="element.toLowerCase().includes('met')" src="@/modules/account/img/metx-logo.svg"
                            class="inline-block h-7 w-7 mx-2 border-2 rounded-3xl">
                          ...
                          <a :href="explorerLink(displayAssetId(element))" target=_new
                            v-else-if="element.split(' ').length == 2">
                            <div class="text-gray-400 cursor-pointer">{{ displayAssetId(element) }}</div>
                          </a>
                          <a :href="explorerLink(displayAssetId(element))" target=_new v-else>
                            <div class="text-blue-600 cursor-pointer">{{ displayLinkNamespace(element) }}</div>
                          </a>
                        </div>
                      </div>
                    </div>
      ...
      <div class="flex items-center h-14 lg:h-28 justify-center" v-if="!isSigned">
        <router-link
          :to='{ name: "ViewAccountPendingTransactions", params: { address: currentAccount ? currentAccount.address : "" } }'
          class="text-gray-600 bg-white px-5 py-2 lg:px-10 lg:py-3 rounded-md text-xs lg:text-tsm inline-block border-2 border-gray-200 mr-5">{{
            $t('transaction.doThisLater') }}</router-link>
        <CosignPasswordModal :transactionHash='txnHash' :disabled="invalidCosigner || isSigned"
          @return-password="signAggTxn" />
      </div>
      <div class="flex items-center h-14 lg:h-28 justify-center" v-else>
        <router-link
          :to='{ name: "ViewAccountPendingTransactions", params: { address: currentAccount ? currentAccount.address : "" } }'
          class="text-gray-600 bg-white px-5 py-2 lg:px-10 lg:py-3 rounded-md text-xs lg:text-tsm inline-block border-2 border-gray-200 mr-5">{{
            $t('general.close') }}</router-link>
      </div>
    </div>
  </div>
</template>
...
import { Account, Address, Password, TransactionType, PublicAccount, Transaction, ModifyMultisigAccountTransaction, type InnerTransaction } from "tsjs-xpx-chain-sdk";
...
import { DashboardService, InnerTxnLegendType, MsgType, type InnerTxnDetails, type TxnDetails } from '@/modules/dashboard/service/dashboardService';
...
const innerTransactions = ref<myInnerTxnDetails[]>([]);
const innerTransactionsSimple = ref<{ Type: string, PublicKey: string, Address: string }[]>([]);
const invalidCosigner = ref(false);
const isSigned = ref(false);

let aggregateTxn: Transaction | null = null;

let allCosigners: string[] = []; // hold all the final cosigners public Keys
let pendingCosigners: string[] = []; // pending final cosigners to sign
...
let innerSignedList: boolean[] = []; // true, false list - innerTxn is fully signed
let innerRelatedList: boolean[] = []; // true, false list - related to current account
...
let signedSigners: string[] = []; // all the cosigned final signers + initiator, include multisig account (calculated)
...
let displaySDA = (asset: string) => {
  let SDAAmount = asset.split(" ")
  return Helper.toCurrencyFormat(parseFloat(SDAAmount[0]), 0)
}

let displayAssetId = (asset: string) => {
  let assetId = asset.split(" ")
  return assetId[1]
}

let displayLinkNamespace = (asset: string) => {
...
const checkCosigner = () => {
  ...
  let publicKey = currentAccount.value.publicKey
  let foundCosigner = allCosigners.find(cosigner => cosigner === publicKey);
  if (foundCosigner === undefined) {
    invalidCosigner.value = true;
  }
  else {
    let foundSigned = signedSigners.find(cosigner => cosigner === publicKey);
...
interface myInnerTxnDetails extends InnerTxnDetails {
  infoInfoList?: TxnDetails[],
  infoList?: TxnDetails[],
  infoGreenList?: TxnDetails[],
  infoRedList?: TxnDetails[]
}

const allTxnCosigners = ref<string[]>([]);
let castToMultisigTxn = (txn: InnerTransaction) => {
  return txn as ModifyMultisigAccountTransaction
}
const loadPartialTransaction = async () => {

  if (!dashboardService.value || !AppState.chainAPI || !walletState.currentLoggedInWallet) {
    return
  }
  try {
    aggregateTxn = await AppState.chainAPI.transactionAPI.getPartialTransaction(p.txnHash);
    if (!aggregateTxn.signer) {
    ...
    let allInnerTransactions: myInnerTxnDetails[] = [];

    for (let i = 0; i < castedAggregateTxn.innerTransactions.length; ++i) {
      let extractedData: myInnerTxnDetails | null = await dashboardService.value.extractPartialInnerTransaction(castedAggregateTxn.innerTransactions[i]);
      if (!extractedData) {
      ...
      let currentInnerSigners: string[] = [];
      ...
      if (castedAggregateTxn.innerTransactions[i].type === TransactionType.MODIFY_MULTISIG_ACCOUNT) {
        ...
        let allDeepCosigners = await CosignUtils.getAllDeepModifyMultisigCosigners(castToMultisigTxn(txn));
        currentInnerSigners = allDeepCosigners;

        let flatCosigners = await CosignUtils.getAllFlatModifyMultisigCosigners(castToMultisigTxn(txn));
        ...
        innerRelatedList.push(allDeepCosigners.includes(currentAccount.value.publicKey));
        ...
        innerRelatedList.push(currentInnerSigners.includes(currentAccount.value.publicKey));
...
const isHasSigned = (cosignerPublicKey: string) => {
...
const displayAccountLabel = (cosignerPublicKey: string) => {
  let cosignerAddress = Address.createFromPublicKey(cosignerPublicKey, AppState.networkType);
  return convertCosignerAddressToName(cosignerAddress.plain());
}

const displayAccountAddress = (cosignerPublicKey: string) => {
  let cosignerAddress = Address.createFromPublicKey(cosignerPublicKey, AppState.networkType).pretty();
  return cosignerAddress;
}

const convertCosignerAddressToName = (address: string) => {
  if (!walletState.currentLoggedInWallet) {
    return
  }
  let aliasName = "";
...
const convertName = (address: string) => {
