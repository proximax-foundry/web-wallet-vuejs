import {
  listenerState,
  AddressAggregateTransactions,
  AddressConfirmedTransactions,
  AddressCosignatureSignedTransactions,
  AddressTransactionStatuses,
...
const aggregateTxCountDownSet: boolean = false;
...
export class ListenerStateUtils {
  static addAutoAnnounceSignedTransaction(
    autoAnnounceSignedTransaction: AutoAnnounceSignedTransaction
  ): void {
    const exist = listenerState.autoAnnounceSignedTransaction.find(
      (autoTx) =>
        autoTx.signedTransaction.hash ===
        autoAnnounceSignedTransaction.signedTransaction.hash
    );
    ...
    listenerState.autoAnnounceSignedTransaction.push(
      autoAnnounceSignedTransaction
    );
  }

  static addBlock(blockInfo: BlockInfo): void {
    const exist = listenerState.newBlocks.find(
      (block) => block.height.compact() === blockInfo.height.compact()
    );
    ...
    const currentHeight = blockInfo.height.compact();
    listenerState.currentBlock = currentHeight;

    ListenerStateUtils.fireAutoAnnounce();
  }

  static addConfirmedTransactions(
    transaction: Transaction,
    address: Address
  ): void {
    let addressConfirmedTransactions = listenerState.confirmedTransactions.find(
      (x) => x.address === address.plain()
    );

    if (addressConfirmedTransactions) {
      const existingTx =
        addressConfirmedTransactions.confirmedTransactions.find(
          (tx) => tx.transactionInfo?.hash === transaction.transactionInfo?.hash
        );

      if (!existingTx) {
        const index = listenerState.confirmedTransactions.findIndex(
          (x) => x.address === address.plain()
        );
        addressConfirmedTransactions.confirmedTransactions.push(transaction);
        listenerState.confirmedTransactions[index] =
          addressConfirmedTransactions;
      }
    } else {
      addressConfirmedTransactions = new AddressConfirmedTransactions(
        address.plain()
      );
      addressConfirmedTransactions.confirmedTransactions.push(transaction);
    ...
    if (transaction.transactionInfo && transaction.transactionInfo.hash) {
      listenerState.allConfirmedTransactionsHash.push(
  ...
  static addUnconfirmedTransactions(
    transaction: Transaction,
    address: Address
  ): void {
    let addressUnconfirmedTransactions =
      listenerState.unconfirmedTransactions.find(
        (x) => x.address === address.plain()
      );

    if (addressUnconfirmedTransactions) {
      const existingTx =
        addressUnconfirmedTransactions.unconfirmedTransactions.find(
          (tx) => tx.transactionInfo?.hash === transaction.transactionInfo?.hash
        );

      if (!existingTx) {
        const index = listenerState.unconfirmedTransactions.findIndex(
          (x) => x.address === address.plain()
        );
        addressUnconfirmedTransactions.unconfirmedTransactions.push(
          transaction
        );
        listenerState.unconfirmedTransactions[index] =
          addressUnconfirmedTransactions;
      }
    } else {
      addressUnconfirmedTransactions = new AddressUnconfirmedTransactions(
        address.plain()
      );
      addressUnconfirmedTransactions.unconfirmedTransactions.push(transaction);
      listenerState.unconfirmedTransactions.push(
        addressUnconfirmedTransactions
      );
    }
    ...
    // console.log("Address: "+ address.plain());
    if (transaction.transactionInfo && transaction.transactionInfo.hash) {
      listenerState.allUnconfirmedTransactionsHash.push(
        transaction.transactionInfo.hash
      );
    }
  ...
  static removeUnconfirmedTransactions(
    transactionHash: string,
    address: Address
  ): void {
    const addressUnconfirmedTransactions =
      listenerState.unconfirmedTransactions.find(
        (x) => x.address === address.plain()
      );

    if (addressUnconfirmedTransactions) {
      const existingTxIndex =
        addressUnconfirmedTransactions.unconfirmedTransactions.findIndex(
          (tx) => tx.transactionInfo?.hash === transactionHash
        );

      if (existingTxIndex >= 0) {
        addressUnconfirmedTransactions.unconfirmedTransactions.splice(
          existingTxIndex,
          1
        );
        const index = listenerState.unconfirmedTransactions.findIndex(
  ...
  static addTransactionStatus(
    transactionStatusError: TransactionStatusError,
    address: Address
  ): void {
    let addressTransactionStatuses = listenerState.transactionStatus.find(
      (x) => x.address === address.plain()
    );

    if (addressTransactionStatuses) {
      const existingStatus = addressTransactionStatuses.transactionStatus.find(
        (tx) => tx.hash === transactionStatusError.hash
      );

      if (!existingStatus) {
        const index = listenerState.transactionStatus.findIndex(
          (x) => x.address === address.plain()
        );
        addressTransactionStatuses.transactionStatus.push(
          transactionStatusError
        );
        listenerState.transactionStatus[index] = addressTransactionStatuses;
      }
    } else {
      addressTransactionStatuses = new AddressTransactionStatuses(
        address.plain()
      );
  ...
  static addAggregateBonded(
    transaction: AggregateTransaction,
    address: Address
  ): void {
    let addressAggregateTransaction =
      listenerState.aggregateBondedTransaction.find(
        (x) => x.address === address.plain()
      );

    if (addressAggregateTransaction) {
      const existingTransaction =
        addressAggregateTransaction.aggregateBonded.find(
          (tx) => tx.transactionInfo?.hash === transaction.transactionInfo?.hash
        );
      ...
      if (!existingTransaction) {
        ...
        listenerState.aggregateBondedTransaction[index] =
          addressAggregateTransaction;
      }
    } else {
      addressAggregateTransaction = new AddressAggregateTransactions(
        address.plain()
      );
      addressAggregateTransaction.aggregateBonded.push(transaction);
      listenerState.aggregateBondedTransaction.push(
        addressAggregateTransaction
      );
    }
    ...
    //ListenerStateUtils.updateAutoAnnounceByConfirmedHash(transaction.transactionInfo.hash);
    if (transaction.transactionInfo && transaction.transactionInfo.hash) {
      listenerState.allAggregateBondedTransactionHash.push(
        transaction.transactionInfo.hash
      );
    }
  ...
  static removeAggregateBonded(
    transactionHash: string,
    address: Address
  ): void {
    const addressAggregateBondedTransaction =
      listenerState.aggregateBondedTransaction.find(
        (x) => x.address === address.plain()
      );
    ...
    if (addressAggregateBondedTransaction) {
      const existingTxIndex =
        addressAggregateBondedTransaction.aggregateBonded.findIndex(
          (tx) => tx.transactionInfo?.hash === transactionHash
        );

      if (existingTxIndex >= 0) {
        addressAggregateBondedTransaction.aggregateBonded.splice(
          existingTxIndex,
          1
        );
        const index = listenerState.aggregateBondedTransaction.findIndex(
          (x) => x.address === address.plain()
        );
        listenerState.aggregateBondedTransaction[index] =
          addressAggregateBondedTransaction;
      }
    }
  ...
  static addCosignatureSignedTransaction(
    cosignatureSignedTransaction: CosignatureSignedTransaction,
    address: Address
  ): void {
    let addressCosignatureAdded = listenerState.cosignatureAdded.find(
      (x) => x.address === address.plain()
    );
    ...
    if (addressCosignatureAdded) {
      const existingTransaction = addressCosignatureAdded.cosignatureAdded.find(
        (tx) =>
          tx.parentHash === cosignatureSignedTransaction.parentHash &&
          tx.signer === cosignatureSignedTransaction.signer
      );

      if (!existingTransaction) {
        const index = listenerState.cosignatureAdded.findIndex(
          (x) => x.address === address.plain()
        );
        addressCosignatureAdded.cosignatureAdded.push(
          cosignatureSignedTransaction
        );
        listenerState.cosignatureAdded[index] = addressCosignatureAdded;
      }
    } else {
      addressCosignatureAdded = new AddressCosignatureSignedTransactions(
        address.plain()
      );
      addressCosignatureAdded.cosignatureAdded.push(
        cosignatureSignedTransaction
  ...
  static removeCosignatureSignedTransactionByHash(
    transactionHash: string
  ): void {
    ...
    listenerState.cosignatureAdded.forEach((addressCosignature) => {
      const filteredCosignature = addressCosignature.cosignatureAdded.filter(
        (x) => x.parentHash !== transactionHash
      );
    ...
    allCosignatureAddedTx = allCosignatureAddedTx.filter(
      (tx, index, array) =>
        ...
        array.findIndex(
          (foundTx) =>
            foundTx.signer === tx.signer && foundTx.parentHash === tx.parentHash
        )
    );
  ...
  static handleConfirmedRecount() {
    ...
    listenerState.allConfirmedTransactionsHash = Array.from(
      new Set(listenerState.allConfirmedTransactionsHash)
    );

    listenerState.confirmedTxLength =
      listenerState.allConfirmedTransactionsHash.length;
  }

  static handleUnconfirmedRecount() {
    confirmedCountDownSet = false;

    listenerState.allUnconfirmedTransactionsHash = Array.from(
      new Set(listenerState.allUnconfirmedTransactionsHash)
    );

    listenerState.unconfirmedTxLength =
      listenerState.allUnconfirmedTransactionsHash.length;
  }

  static handleAggregateTxRecount() {
    confirmedCountDownSet = false;

    listenerState.allAggregateBondedTransactionHash = Array.from(
      new Set(listenerState.allAggregateBondedTransactionHash)
    );

    listenerState.aggregateBondedTxLength =
      listenerState.allAggregateBondedTransactionHash.length;
  }
  ...
  static handleCosignatureRecount() {
    ...
    listenerState.allCosignatureAdded =
      listenerState.allCosignatureAdded.filter(
        (tx, index, array) =>
          index ===
          array.findIndex(
            (foundTx) =>
              foundTx.signer === tx.signer &&
              foundTx.parentHash === tx.parentHash
          )
      );

    listenerState.cosignatureAddedTxLength =
      listenerState.allCosignatureAdded.length;
  }
  ...
  static handleRemoveUnconfirmedRecount() {
    ...
    listenerState.allUnconfirmedTransactionsHash =
      listenerState.allUnconfirmedTransactionsHash.filter(
        (txHash) => !unconfirmedToRemoveHash.includes(txHash)
      );
  ...
  static handleTxStatusRecount() {
    ...
    listenerState.allTransactionStatus =
      listenerState.allTransactionStatus.filter(
        (tx, index, array) =>
          index === array.findIndex((foundTx) => foundTx.hash === tx.hash)
      );

    console.log(listenerState.allTransactionStatus);

    listenerState.transactionStatusLength =
      listenerState.allTransactionStatus.length;
  }
  ...
  static handleRemoveAggregateBondedRecount() {
    ...
    listenerState.allAggregateBondedTransactionHash =
      listenerState.allAggregateBondedTransactionHash.filter(
        (txHash) => !aggregateBondedHashToRemove.includes(txHash)
      );
  ...
  static checkingAutoAnnounce() {
    ...
    const currentBlockHeight = listenerState.currentBlock;

    for (
      let i = 0;
      i < listenerState.autoAnnounceSignedTransaction.length;
      ++i
    ) {
      let letAnnouce = false;

      const currentAutoAnnounceTx =
        listenerState.autoAnnounceSignedTransaction[i];
      ...
      if (
        ...
        currentAutoAnnounceTx.hashAnnounceBlock.hashFound &&
    ...
    const remainingTransactionsToAnnounce =
    ...
    listenerState.autoAnnounceSignedTransaction =
      remainingTransactionsToAnnounce;
  }
  ...
  static updateAutoAnnounceByConfirmedHash(hash: string, blockHeight?: number) {
    for (
      ...
      const current = listenerState.autoAnnounceSignedTransaction[i];
      ...
      if (current.hashAnnounceBlock) {
        if (current.hashAnnounceBlock.trackHash === hash) {
          ...
          if (blockHeight) {
            ...
            current.hashAnnounceBlock.hashFoundAtBlock =
              listenerState.currentBlock;
          }
        }
      }
