...
export class NetworkStateUtils {
  static refreshAvailableNetwork(): void {
    networkState.availableNetworks =
      ChainProfileNames.createDefault().names.map((value) => value.name);
  }
  ...
  static updateNetworkProfile(): void {
    ...
    AppState.nativeToken.divisibility =
      chainProfile.network.currency.divisibility;
    AppState.nativeToken.label = chainProfile.network.currency.name;
    AppState.nativeToken.fullNamespace =
      chainProfile.network.currency.namespace;
    ...
    AppState.txnSwapLog = [];
    if (
      AppState.networkType === NetworkType.PRIVATE ||
      AppState.networkType === NetworkType.PRIVATE_TEST
    ) {
    ...
    const chainProfileConfig = new ChainProfileConfig(
  ...
  static setAPINodeInit(): void {
    const chainProfilePreferences = new ChainProfilePreferences(
      networkState.chainNetworkName
    );

    chainProfilePreferences.init();

    const endpoints = networkState.currentNetworkProfile
      ? networkState.currentNetworkProfile.apiNodes
      : [];

    if (
      chainProfilePreferences.apiNode &&
      endpoints.includes(chainProfilePreferences.apiNode)
    ) {
    ...
    const portNumber = networkState.currentNetworkProfile?.httpPort;

    SessionService.setNumber("nodePort", portNumber ? portNumber : 3000);
    SessionService.setRaw(
      "selectedChainNode",
      networkState.selectedAPIEndpoint
    );

    AppState.nodeFullURL = NetworkStateUtils.buildAPIEndpointURL(
      networkState.selectedAPIEndpoint
    );
    AppState.nodeURL = networkState.selectedAPIEndpoint;
    AppState.wsNodeFullURL = NetworkStateUtils.buildWSEndpointURL(
      networkState.selectedAPIEndpoint
    );
  ...
  static buildAPIEndpointURL(url: string): string {
    const portNumber =
      networkState.currentNetworkProfile !== null
  ...
  static checkDefaultNetwork(): void {
    ...
    const sessionSelectedNetworkName =
      sessionStorage.getItem(sessionNetworkName);
