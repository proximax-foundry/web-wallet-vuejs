<template>
  <div class="flex flex-col justify-between md:min-h-screen bg-navy-primary" @click="clickEvent">
    ...
    <div :class="login?`flex min-full-screen`:``">
      <NavigationMenu v-if="login" class="md:mt-16 flex-shrink-0 bg-navy-primary text-left text-xs bg-navi z-10 absolute md:fixed inset-y-0 left-0 transform md:-translate-x-0 transition duration-200 ease-in-out" :class="`${isShowNavi?'-translate-x-0':'-translate-x-full'}`"></NavigationMenu>
      <div :class="`${ login?'inline-block flex-grow':''}`">
        <div :class="`${ login?'flex flex-col min-full-screen bg-white':''}`">
          <router-view class="md:ml-60 mt-12 md:mt-16 flex-grow" v-if="currentRouteName=='ViewDashboard'"></router-view>
          <router-view class="md:ml-60 mt-12 md:mt-16 flex-grow px-2 sm:px-10 pt-5" v-else :key="$route.path"></router-view>
    ...
    <footer class="mx-auto h-12 mt-20 text-center sm:text-justify sm:flex text-txs md:text-xs sm:justify-between container text-white pb-5" v-if="!login">
