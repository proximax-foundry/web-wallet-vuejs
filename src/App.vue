      ...
      <template #message="slotProps">
        <div style="width: 100%" class="grid grid-cols-12">
          ...
          <div class="col-span-12">
            ...
            <div :style="`font-size: ${slotProps.message.detail3RemSize ? slotProps.message.detail3RemSize : 1}rem`"
              v-if="slotProps.message.url">
              <a :href="slotProps.message.url" target="_blank">
                {{ slotProps.message.detail3 }}
              </a>
            </div>
            <div :style="`font-size: ${slotProps.message.detail3RemSize ? slotProps.message.detail3RemSize : 1}rem`"
              v-else>{{ slotProps.message.detail3 }}</div>
          </div>
        </div>
      </template>
    </Toast>
      ...
      <template #message="slotProps">
        <div style="width: 100%" class="grid grid-cols-12">
          <div class="col-span-2">
            ...
            <i v-else-if="slotProps.message.severity === 'error'" class="pi pi-times-circle"
              style="font-size: 2.5rem"></i>
            <i v-else-if="slotProps.message.severity === 'info'" class="pi pi-info-circle" style="font-size: 2.5rem"></i>
            <i v-else-if="slotProps.message.severity === 'warn'" class="pi pi-exclamation-circle"
              style="font-size: 2.5rem"></i>
          </div>
          <div class="col-span-10">
            <div class="font-semibold">{{ slotProps.message.summary }}</div>
        ...
        <div style="width: 100%" class="grid grid-cols-12">
          ...
          <div class="col-span-12">
            ...
            <div :style="`font-size: ${slotProps.message.detail3RemSize ? slotProps.message.detail3RemSize : 1}rem`"
              v-else>{{ slotProps.message.detail3 }}</div>
            <div
              :style="`font-size: ${slotProps.message.detail4 && slotProps.message.detail4.length > 64 ? 0.4 : 0.5}rem`">
              {{ slotProps.message.detail4 }}</div>
          </div>
        </div>
      </template>
    </Toast>
...
import { computed, getCurrentInstance, provide, watch, ref, reactive, onUnmounted, onMounted } from "vue";
