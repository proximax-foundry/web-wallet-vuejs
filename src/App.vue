<template>
  <div class="flex flex-col justify-between md:min-h-screen bg-navy-primary" @click="clickEvent">
    ...
    <div :class="login?`flex min-full-screen`:``">
      <NavigationMenu v-if="login" class="lg:mt-16 flex-shrink-0 bg-navy-primary text-left text-xs bg-navi z-10 absolute lg:fixed inset-y-0 left-0 transform lg:-translate-x-0 transition duration-200 ease-in-out" :class="`${isShowNavi?'-translate-x-0':'-translate-x-full'}`"></NavigationMenu>
