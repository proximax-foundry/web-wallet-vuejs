<template>
  <div class="p-3 text-center">
    <img :src="`${require('../assets/img/services/' + service.img)}`" style="width: 80px; height: 80px;" class="inline-block mb-3" :class="(service.enable)?'':'filtergrayscale'">
      ...
      <div :class="showMenuCall?'':'hidden'" class="absolute w-40 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
        <div class="py-1" role="none">
          <div v-for="(option, item) in service.menu" :key="item">
            <router-link v-if="option.link!=''" :to="option.link" class="block px-2 py-1 text-xs text-gray-700 hover:bg-blue-primary hover:text-white" role="menuitem">{{ option.name }}</router-link>
