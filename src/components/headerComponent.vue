  ...
  <header class="z-10 fixed w-full">
    <div class="header-height flex items-stretch lg:pr-2 bg-gray-50 filter drop-shadow-xl" v-if="loginStatus">
      <div class="w-12 lg:w-16 flex flex-row items-center left-gray-line lg:hidden ">
        <div class="text-center w-full h-6" @mouseover="hoverOverNavigation" @mouseout="hoverOutNavigation">
          <img src="@/assets/img/icon-menu.svg" class="h-4 w-4 opacity-80 hover:opacity-100 inline-block cursor-pointer"
            @click="toggleSidebar">
        </div>
      </div>
      ...
      <div class="flex flex-row ml-auto h-full">
        <div class="hidden lg:flex w-16 flex-row items-center left-gray-line relative">
          <div class="text-center w-full h-7 cursor-pointer" @mouseover="setHoverSupportToTrue"
            @mouseout="setHoverSupportToFalse">
            <img src="@/assets/img/icon-support-contact.svg"
          ...
          <div
            ...
            style="top: 60px" v-if="isShowSupport" @mouseover="isShowSupport = true; isHoverSupportPanel = true;"
            @mouseout="hideSupportPanel">
            <div class="font-bold p-2 text-txs uppercase">{{ $t('home.beginnerGuide') }}</div>
            <div class="grid grid-cols-2">
              <div>
                <div class="p-2">
                  <a class="mb-2 block text-blue-primary" href="https://bcdocs.xpxsirius.io/"
                    target=_new>{{ $t('home.gettingStarted') }}<img src="@/assets/img/icon-open_in_new.svg"
                      class="inline-block absolute -top-1 ml-2"></a>
                  <div class="text-txs h-10">{{ $t('home.allInfo') }}</div>
                </div>
                <div class="p-2">
                  <a class="mb-2 block text-blue-primary"
                    href="https://bcdocs.xpxsirius.io/docs/getting-started/what-is-proximax-sirius-chain/"
                    target=_new>{{ $t('home.siriusChain') }}<img src="@/assets/img/icon-open_in_new.svg"
                      class="inline-block absolute -top-1 ml-2"></a>
              ...
              <div>
                <div class="p-2">
                  <a class="mb-2 block text-blue-primary"
                    ...
                    target=_new>{{ $t('general.namespaceQues') }}<img src="@/assets/img/icon-open_in_new.svg"
                      class="inline-block absolute -top-1 ml-2"></a>
                ...
                <div class="p-2">
                  <a class="mb-2 block text-blue-primary"
                    ...
                    target=_new>{{ $t('general.assetQues') }}<img src="@/assets/img/icon-open_in_new.svg"
                      class="inline-block absolute -top-1 ml-2"></a>
            ...
            <div class="w-full p-2 pt-3 border-t border-gray-100">
              <a href="https://t.me/proximaxhelpdesk" target=_new
                class="text-xs text-blue-primary">{{ $t('home.contactTeam') }}</a>
            </div>
          </div>
        </div>
          ...
          <router-link :to="{ name: 'ViewNotification' }" class="text-center w-full h-7 relative">
            ...
            <span class="flex items-center justify-center absolute notification_counter" v-else>
              <span v-if="newNotificationCount > 0"
                class="relative inline-flex rounded-full z-20 h-4 w-4 bg-orange-primary text-xxs text-white items-center justify-center">{{
        ...
        <div class="hidden w-40 pl-3 text-center lg:flex items-center left-gray-line">
          <div class="lg:flex lg:items-center">
            <img src="@/assets/img/icon-testnet-block.svg" class="w-3 lg:w-7 block lg:inline-block"
              :title="chainAPIEndpoint" v-if="wideScreen">
            <div class="block lg:inline-block text-txs text-white text-left lg:ml-2">
              <div class="text-xxs lg:text-tsm text-navy-primary">{{ networkState.chainNetworkName }}</div>
            </div>
          </div>
        </div>
    ...
    <div class="container mx-auto header-height flex items-stretch bg-navy-primary" v-else>
      <div class="flex-none self-center flex items-end ml-2 lg:ml-0">
        <router-link :to="loginStatus ? { name: 'ViewDashboard' } : { name: 'Home' }"><img
            src="@/assets/img/sirius-logo-whitetext.svg" class="w-24 lg:w-40"></router-link>
      </div>
      <div class="flex-grow"></div>
      <div class="flex-none self-center header-menu">
        <div class="w-20 inline-block ml-2 sm:ml-0" v-if="wideScreen">
          <router-link :to="{ name: 'Home' }"
            class="font-normal hover:font-bold inline-block text-white">{{ $t('home.home') }}</router-link>
        </div>
        <div class="w-20 inline-block" v-if="wideScreen">
          <router-link :to="{ name: 'ViewWallets' }"
            class="hover:font-bold text-white">{{ $t('general.wallet', 2) }}</router-link>
        </div>
    ...
    <div v-if="!wideScreen && !loginStatus" class="py-1 text-center" style="background: #484A69">
      <div class="w-20 text-center inline-block text-gray-300">
        <router-link :to="{ name: 'Home' }"
          class="font-normal hover:font-bold inline-block text-xs sm:text-sm">{{ $t('home.home') }}</router-link>
      </div>
      <div class="w-20 text-center inline-block text-gray-300">
        <router-link :to="{ name: 'ViewWallets' }"
          class="hover:font-bold text-xs sm:text-sm">{{ $t('general.wallet', 2) }}</router-link>
      </div>
    </div>
  </header>
</template>
...
import { computed, getCurrentInstance, inject, ref, watch, onUnmounted, onMounted } from "vue";
...
let notificationTimeout = ref<NodeJS.Timeout | number>(0);
let doRecheckAssetsNamesInterval = ref<NodeJS.Timer | number>(0);
let doConfirmedTxnCheckingInterval = ref<NodeJS.Timer | number>(0);
let doTxnCheckingInterval = ref<NodeJS.Timer | number>(0);
...
const doLogin = async () => {
  if (loginStatus.value && AppState.isReady && walletState.currentLoggedInWallet && networkState.currentNetworkProfile) {
    await WalletUtils.refreshAllAccountDetails(walletState.currentLoggedInWallet, networkState.currentNetworkProfile);
    doGetNotification();
    //connectListener();
  }
  else if (!loginStatus.value || !AppState.isReady || !walletState.currentLoggedInWallet || !networkState.currentNetworkProfile) {
...
const doConfirmedTxnChecking = async () => {
  ...
  if (loginStatus.value && AppState.isReady) {
    ...
    doConfirmedTxnCheckingInterval.value = setInterval(WalletUtils.checkConfirmedTxnChecking, 60000); // 1 minute
  }
  else if (loginStatus.value && !AppState.isReady) {
...
const doRecheckAssetsNames = async () => {
  ...
  if (loginStatus.value && AppState.isReady) {
    ...
    doRecheckAssetsNamesInterval.value = setInterval(WalletUtils.recheckAssetsNames, 60000 * 5); // 5 minute
  }
  else if (loginStatus.value && !AppState.isReady) {
...
const checkTxnStatus = async () => {
  ...
  for (let i = 0; i < transactionStatuses.length; ++i) {
    ...
    if (txnsHash1.includes(transactionStatus.hash)) {
      let txnActivity = AppState.txnActivityLog.find(x => x.txnHash === transactionStatus.hash);
      if (!txnActivity) {
        continue
      }
      if (txnActivity.status !== transactionStatus.group && transactionStatus.group) {
        ...
        addTxnToastMessage(txnActivity.status, transactionStatus.hash, txnActivity.statusMsg);
      }
    }
    else if (txnsHash2.includes(transactionStatus.hash)) {
      let txnCosign = AppState.txnCosignLog.find(x => x.txnHash === transactionStatus.hash);
      if (!txnCosign) {
        continue
      }
      if (txnCosign.status !== transactionStatus.group && transactionStatus.group) {
        ...
        if (!displayedTxn.includes(transactionStatus.hash)) {
          addTxnToastMessage(txnCosign.status, transactionStatus.hash, txnCosign.statusMsg);
        }
      }
    }
    else {
      let txnSwap = AppState.txnSwapLog.find(x => x.txnHash === transactionStatus.hash);
      if (!txnSwap) {
        continue
      }
      if (txnSwap.status !== transactionStatus.group && transactionStatus.group) {
        ...
        addTxnToastMessage(txnSwap.status, transactionStatus.hash, txnSwap.statusMsg, "swap");
      }
    }
  }
...
const doTxnChecking = async () => {
  if (loginStatus.value && AppState.isReady) {
    ...
    doTxnCheckingInterval.value = setInterval(checkTxnStatus, targetBlockSeconds.value * 1000);
  }
  else if (loginStatus.value && !AppState.isReady) {
    ...
    if (currentAutoAnnounceTx.announceAtBlock && currentBlockHeight >= currentAutoAnnounceTx.announceAtBlock) {
      ...
      try {
        ...
        if (txnStatusInfo.group === transactionGroupType.CONFIRMED) {
          ...
          currentAutoAnnounceTx.hashAnnounceBlock.hashFoundAtBlock = txnStatusInfo.height?txnStatusInfo.height.compact():0
...
const addTxnToastMessage = (status :string, txnHash :string, statusMessage :string, extraData = "") => {
...
onUnmounted(() => {
  window.removeEventListener("resize", headerMenuHandler);
})
onMounted(() => {
  headerMenuHandler()
  window.addEventListener("resize", headerMenuHandler);
})
