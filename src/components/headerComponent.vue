<template>
  <header class="z-10 fixed w-full">
    <div
      class="header-height flex items-stretch lg:pr-2 bg-gray-50 filter drop-shadow-xl"
      v-if="loginStatus"
    >
      <div
        class="w-12 lg:w-16 flex flex-row items-center left-gray-line lg:hidden"
      >
        <div
          class="text-center w-full h-6"
          @mouseover="hoverOverNavigation"
          @mouseout="hoverOutNavigation"
        >
          <img
            src="@/assets/img/icon-menu.svg"
            class="h-4 w-4 opacity-80 hover:opacity-100 inline-block cursor-pointer"
            @click="toggleSidebar"
          />
        </div>
      </div>
      <div
        class="flex justify-center items-center lg:bg-navy-primary logo-header"
      >
        <router-link
          :to="loginStatus ? { name: 'ViewDashboard' } : { name: 'Home' }"
        >
          <img
            src="@/assets/img/sirius-logo-whitetext.svg"
            class="w-24 -ml-4 tsm:w-40 hidden lg:inline-block"
          />
        </router-link>
      </div>
      <div
        class="flex items-center ml-auto mr-28 lg:mr-auto lg:ml-10 logo-header"
      >
        ...
        >
          <img
            src="@/assets/img/sirius-logo-blacktext.svg"
            class="w-40 lg:hidden"
          />
        </router-link>
      </div>
      ...
      <div class="flex flex-row ml-auto h-full">
                ...
                <div class="p-2">
                  <a
                    ...
                    >{{ $t("home.gettingStarted")
                      ...
                      class="inline-block absolute -top-1 ml-2"
                  /></a>
        ...
        <div class="w-12 lg:w-16 flex flex-row items-center left-gray-line">
          <router-link
            :to="{ name: 'ViewNotification' }"
            class="text-center w-full h-7 relative"
          >
            <span
              class="flex h-5 w-5 items-center justify-center absolute notification_counter"
            ...
            <span
              class="flex items-center justify-center absolute notification_counter"
              v-else
            >
              <span
        ...
        <div
          class="hidden lg:flex w-16 lg:flex-row items-center left-gray-line"
        >
          <div class="text-center w-full h-4 lg:h-6">
            <router-link
              :to="{ name: 'ViewSettings' }"
              class="h-7 w-4 lg:h-6 lg:w-6 inline-block"
            >
              <img
        ...
        <div
          class="hidden w-40 pl-3 text-center lg:flex items-center left-gray-line"
        >
          <div class="lg:flex lg:items-center">
            <img
              src="@/assets/img/icon-testnet-block.svg"
              class="w-3 lg:w-7 block lg:inline-block"
              :title="chainAPIEndpoint"
              v-if="wideScreen"
            />
            <div
              class="block lg:inline-block text-txs text-white text-left lg:ml-2"
            >
    ...
    <div
      class="container mx-auto header-height flex items-stretch bg-navy-primary"
      v-else
    >
      ...
      <div class="flex-none self-center header-menu">
        <div class="w-20 inline-block ml-2 sm:ml-0" v-if="wideScreen">
          <router-link
            :to="{ name: 'Home' }"
            class="font-normal hover:font-bold inline-block text-white"
    ...
    <div
      v-if="!wideScreen && !loginStatus"
      class="py-1 text-center"
      style="background: #484a69"
    >
      <div class="w-20 text-center inline-block text-gray-300">
        <router-link
          :to="{ name: 'Home' }"
          class="font-normal hover:font-bold inline-block text-xs sm:text-sm"
          >{{ $t("home.home") }}</router-link
        >
      </div>
      <div class="w-20 text-center inline-block text-gray-300">
        <router-link
          :to="{ name: 'ViewWallets' }"
          class="hover:font-bold text-xs sm:text-sm"
          >{{ $t("general.wallet", 2) }}</router-link
        >
      </div>
    </div>
    ...
    if (
      ...
      try {
        ...
        if (txnStatusInfo.group === transactionGroupType.CONFIRMED) {
          ...
          currentAutoAnnounceTx.hashAnnounceBlock.hashFoundAtBlock =
            txnStatusInfo.height ? txnStatusInfo.height.compact() : 0;
