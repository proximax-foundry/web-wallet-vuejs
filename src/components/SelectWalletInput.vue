<template>
  <div @click='toggleSelection = !toggleSelection' class='border w-8/12 ml-auto mr-auto py-3 px-2 cursor-pointer'>
    <div class='flex'>
      ...
      <div v-if='!toggleSelection && selectedWallet == ""'
      ...
      <img v-if='toggleSelection' @click='selectedWallet = ""' src="@/assets/img/delete.svg"
  ...
  <div class='relative' style='left:16.7%;'>
    <div v-if='toggleSelection' class='absolute border border-t-0 w-8/12  z-50 bg-white max-h-32 overflow-auto '>
      <div v-if='wallets.length > 0' class='pt-2 pl-2 pb-2 font-semibold uppercase'
        style="font-size: 7px ; line-height: 9px;">{{ $t('home.selectWallet') }}</div>
      ...
      <div v-for='(items, index) in wallets' :key="index" class='px-2 py-1 '>
        <div @click='selectWallet(items.label)' class='flex cursor-pointer'>
          ...
          <div v-if='items.label != selectedWallet'
            class='cursor-pointer text-blue-primary text-xxs mt-0.5 ml-auto font-semibold uppercase'>
            {{ $t('general.select') }}</div>
          <div v-if='items.label == selectedWallet' class='text-gray-500 text-xxs mt-0.5 ml-auto uppercase'>
